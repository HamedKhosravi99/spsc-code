"""
MNIST *real-data-calibrated* bandit environment.

Uses MNIST (70,000 samples, 784 pixel features, 10 digit classes).
...
class MNISTEnvironment:
    """
    Real-data-calibrated MNIST bandit.
    ...
    def _load_and_prepare_data(self, target_d, seed):
        """Load MNIST, build features to match target_d."""
        data = fetch_openml('mnist_784', version=1, as_frame=False, parser='auto')
        X_raw = data.data.astype(float)
        y_raw = data.target.astype(int)

        # Remove constant columns (border pixels always 0)
