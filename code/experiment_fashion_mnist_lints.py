"""LinTS baselines on Fashion-MNIST. Same grid/settings as experiment_fashion_mnist_grid.py."""
import os, sys, time, numpy as np
sys.path.insert(0, os.path.dirname(__file__))
from environments import FashionMNISTEnvironment
...
def make_env(seed, d, r):
    return FashionMNISTEnvironment(d=d, r=r, n_actions=40, segment_size=SEG_SIZE,
...
if __name__ == "__main__":
    ...
    print(f"Fashion-MNIST LinTS ({N_SEEDS} seeds)")
    all_results = {}
    for d in D_VALUES:
        for r in R_VALUES:
            if r >= d:
