"""
Warfarin Clinical Dosing Benchmark: Extended SOTA Comparison.

Extends experiment_real_bandit.py (Warfarin portion) by adding:
  - SPSC-Adaptive (Algorithm 4 — no oracle segment boundaries)
  - LowOFUL      (Jun+ '19 — stationary low-rank)
  - VOFUL         (Kim+ '22 — variance-aware low-rank)
  - LowRank-Reward
...
# ---------------------------------------------------------------------------
# Parameters — IDENTICAL to experiment_real_bandit.py (Warfarin)
# ---------------------------------------------------------------------------
D           = 93
K           = 8
T           = 5000
...
def print_table(res, r):
    ...
    print()
    print("=" * 120)
    print(f"  Warfarin d={D}, r={r}, K={K}, T={T}  ({n} seeds)")
    print("-" * 120)
...
def make_figure(all_results, out_path):
    ...
    ax = axes[2]
    # Pick the r with best SPSC/LinUCB ratio
    ...
    fig.suptitle(
        f"Warfarin Extended SOTA — $d={D}$, $K={K}$, $T={T}$\n"
...
if __name__ == "__main__":
    print("=" * 80)
    print("Warfarin Extended SOTA (10 methods)")
    print(f"  d={D}, K={K}, T={T}")
    print(f"  r = {R_VALUES}")
    ...
    # Summary
    print("\n" + "=" * 80)
    print("Warfarin SPSC/LinUCB ratio summary:")
    for r in R_VALUES:
    ...
    make_figure(all_results,
                os.path.join(OUT_DIR, "experiment_warfarin_extended.png"))
    print("\nDone.")
