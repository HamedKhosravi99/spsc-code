"""LinTS baselines on Covertype. Same grid/settings as experiment_covertype_grid.py."""
import os, sys, time, numpy as np
sys.path.insert(0, os.path.dirname(__file__))
from environments import CovtypeEnvironment
from algorithm import LinTS, SWLinTS

N_SEEDS = 10; K = 10; T = 5000
WINDOW = 400; LAM = 0.01; DELTA = 0.05
D_VALUES = [5, 55, 105]; R_VALUES = [1, 10, 20, 30]

def make_env(seed, d, r):
    return CovtypeEnvironment(d=d, r=r, K=K, T=T, n_actions=40, seed=seed * 13 + 7)
...
if __name__ == "__main__":
    ...
    print(f"Covertype LinTS ({N_SEEDS} seeds)")
    all_results = {}
    for d in D_VALUES:
        for r in R_VALUES:
            if r >= d:
    ...
    save_results(__file__,
                 config={"N_SEEDS": N_SEEDS, "K": K, "T": T,
