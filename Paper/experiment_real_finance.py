"""
Experiment: Fama-French 25 Portfolios — Real Financial Bandit.

Real-world data, faithful to the paper's model:
  - theta_t derived from ACTUAL Fama-French daily portfolio returns
  - Reward: y_t = x_t^T theta_t + eps_t  (the paper's linear model)
  - SPSC Algorithm 1 EXACTLY as in the paper (sphere probes, K-inverse)
  - Non-stationarity: quarterly regime changes from real market data
  - Low-rank structure: Fama-French factor model (documented r=2-3)

Compares SPSC Alg 1, LinUCB, D-LinUCB, SW-LinUCB, Oracle-LinUCB.

Outputs:
  experiment_real_finance.png  — 3-panel figure
...
from real_finance_environment import RealFinanceEnvironment
...
class SWLinUCB:
    """Sliding-window LinUCB (Cheung+ NeurIPS 2019)."""
...
class DLinUCB:
    """Discounted LinUCB (Russac et al. NeurIPS 2019)."""
...
def make_env(seed):
    return RealFinanceEnvironment(
        data_dir=DATA_DIR, r=R, start_year=START_YEAR,
        end_year=END_YEAR, segment_days=SEGMENT_DAYS,
        seed=seed * 13 + 7,
    )
    ...
    for seed in range(n_seeds):
        ...
        for name in NAMES:
            ...
            if name == "SPSC-Alg1":
                ...
                m = LinUCB(env, lam=1.0, delta=0.05,
                           seed=seed + 4000).run()
...
def make_figure(results, env_ref, out_path, labels):
    ...
    # Panel (b): Return variance spectrum
    ax = axes[1]
    pca = env_ref.return_pca_spectrum()
    cumvar = np.cumsum(pca)
    n_show = min(10, len(pca))
    x = np.arange(1, n_show + 1)
    ax.bar(x, pca[:n_show], color="#1f77b4", alpha=0.7, label="Variance share")
    ...
    ax.set_ylabel("Fraction of return variance")
    ax.set_title("(b) Return factor structure (SVD)")
    ...
    fig.suptitle(
        f"Fama-French 25 Portfolio Selection  |  "
        f"$d={env_ref.d}$, $r={R}$, {env_ref.K} quarters, "
...
def print_table(results, env_ref, labels):
    print()
    print("=" * 90)
    print("Fama-French 25 Portfolio Selection -- Real Financial Bandit")
    print(f"  d={env_ref.d}, r={R}, K={env_ref.K} quarters, T={env_ref.T}")
    print(f"  sigma_eps={env_ref.sigma_eps:.3f}, S={env_ref.S:.4f}")
    pca = env_ref.return_pca_spectrum()
    print(f"  Return SVD: PC1={pca[0]:.1%}, PC1-2={sum(pca[:2]):.1%}, "
          f"PC1-3={sum(pca[:3]):.1%}")
    print("-" * 90)
...
if __name__ == "__main__":
    print("=" * 60)
    print("Fama-French 25 -- Real Financial Bandit")
    print("  Using SPSC Algorithm 1 (sphere probes + K-inverse)")
    print("=" * 60)

    env_ref = make_env(0)
    d = env_ref.d
    print(f"  d={d}, r={R}, K={env_ref.K} quarters, T={env_ref.T}")
    print(f"  sigma_eps={env_ref.sigma_eps:.3f}, S={env_ref.S:.4f}")
    ...
    make_figure(results, env_ref,
                os.path.join(OUT_DIR, "experiment_real_finance.png"), labels)
    print("Done.\n")
