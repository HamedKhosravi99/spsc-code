...
\item {\bf Limitations}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Limitations are stated where they arise: the regret bound is conditional on the windowed projected potential condition (\S\ref{sec:theory}); the bound is most informative when within-segment variation $V_{\rm in}$ is small (Theorem~\ref{thm:spsc_regret} discussion); the rule-of-thumb crossover $d-r\gtrsim T^{1/6}$ is qualitative, not sharp (\S\ref{sec:theory}, \S\ref{sec:exp_phase}); SPSC-Adaptive is evaluated empirically only with no formal regret guarantee (\S\ref{sec:theory}).
    \item[] Guidelines:
    \begin{itemize}
