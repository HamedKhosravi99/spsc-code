"""
Satimage *real-data-calibrated* bandit environment.

Uses UCI Satimage (6,430 samples, 36 features, 6 land-cover classes).
All parameters derived from real data:
  - Features: 36 raw spectral features, augmented with pairwise interactions for d > 36
  - theta_k: per-segment OLS fit of features -> class labels
  - Segments: data sorted by class label creates natural non-stationarity
  - Low-rank: 6 land-cover classes with correlated spectral signatures
...
class RealSatimageEnvironment:
    """
    Real-data-calibrated Satimage bandit.
    ...
    def _load_and_prepare_data(self, target_d):
        """Load Satimage, build features to match target_d."""
        data = fetch_openml('satimage', version=1, as_frame=False, parser='auto')
        X_raw = data.data.astype(float)
        y_raw = data.target  # string labels like '1.', '2.', etc.

        # Convert string labels to float
        y_float = np.array([float(y.replace('.', '')) for y in y_raw])

        # Standardize
        scaler = StandardScaler()
        X_std = scaler.fit_transform(X_raw)
        d_raw = X_std.shape[1]  # 36
        ...
        if target_d <= d_raw:
            ...
            X_inter = np.column_stack(interactions)  # 630 interactions
        ...
        sort_idx = np.argsort(y_float)
        self._features = X_full[sort_idx]
        self._labels = (y_float[sort_idx] - y_float.mean())  # center around 0
