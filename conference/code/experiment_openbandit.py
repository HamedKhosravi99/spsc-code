"""
OpenBandit Pipeline real-data experiment.

Runs SPSC and 9 baselines on the ZOZOTOWN fashion bandit logs
(Saito et al. 2020).  This is the **only** dataset in our evaluation
where every (action, reward, context) triple comes from a real logged
exploration policy on a deployed system.

Setup mirrors experiment_pendigits_extended.py:
  - K=10 segments (time-buckets)
  - T=5,000
...
Output:
  results/experiment_openbandit.json   (per-cell, per-method, per-seed)
  experiment_openbandit.png            (4-panel bar chart)
...
# --- env import (early failure if obp missing) ----------------------------
try:
    from environments.openbandit_environment import (
        OpenBanditEnvironment, OBP_AVAILABLE, OBP_IMPORT_ERROR,
    )
except Exception as e:
    print(f"[FATAL] Failed to import OpenBanditEnvironment: {e}")
    print("Install requirements:  pip install obp scikit-learn")
    sys.exit(1)

if not OBP_AVAILABLE:
    print("=" * 80)
    print("OpenBandit Pipeline (`obp`) is not installed.")
    print("Install with:")
    print("    pip install obp")
    print(f"Original error: {OBP_IMPORT_ERROR}")
    print("=" * 80)
    sys.exit(1)
...
RESULTS_PATH = os.path.join(RESULTS_DIR, "experiment_openbandit.json")
FIGURE_PATH  = os.path.join(OUT_DIR, "experiment_openbandit.png")
...
CAMPAIGN          = "all"      # 'men' | 'women' | 'all'
BEHAVIOR_POLICY   = "random"   # 'random' | 'bts'
...
def make_env(seed, d, r):
    return OpenBanditEnvironment(
        ...
        campaign=CAMPAIGN, behavior_policy=BEHAVIOR_POLICY,
    )
...
def print_cell_summary(res, d, r):
    ...
    print()
    print("=" * 110)
    print(f"  OpenBandit  d={d}, r={r}   ({n} seeds)")
    print("-" * 110)
...
def make_figure(all_results, out_path):
    ...
    fig.suptitle(
        f"OpenBandit (ZOZOTOWN, {CAMPAIGN}/{BEHAVIOR_POLICY}): real "
        f"actions, real exploration, real clicks",
...
def main():
    print("=" * 80)
    print(f"OpenBandit Pipeline experiment "
          f"(campaign={CAMPAIGN}, behavior={BEHAVIOR_POLICY})")
    ...
    # Save (convert tuple keys / numpy arrays to JSON-friendly form)
    serial = {}
