"""
20 Newsgroups *real-data-calibrated* bandit environment.

Uses 20 Newsgroups text corpus — TF-IDF features are naturally high-dimensional
and low-rank (topic/theme structure).

  - Features: TF-IDF of document text, top-d terms by document frequency
  - theta_k: per-segment OLS fit of features -> newsgroup category labels
  - Segments: documents sorted by category creates natural non-stationarity
  - Low-rank: 20 newsgroups cluster into ~5-6 macro-topics
...
class NewsgroupsEnvironment:
    """
    Real-data-calibrated 20 Newsgroups bandit.

    Segments created by sorting documents by category (natural regime changes).
    theta_k = per-segment OLS of TF-IDF features -> category labels.
    ...
    def _load_and_prepare_data(self, target_d):
        """Load 20 Newsgroups, build TF-IDF features to match target_d."""
        dataset = fetch_20newsgroups(subset='all', remove=('headers', 'footers', 'quotes'))
        texts = dataset.data
        y_raw = np.array(dataset.target, dtype=float)

        # Build TF-IDF features — use top max_features terms
        max_features = max(target_d * 2, 500)
        ...
        # Select top target_d features by variance
        variances = X_tfidf.var(axis=0)
        top_idx = np.argsort(-variances)[:target_d]
        X_selected = X_tfidf[:, top_idx]
        ...
        X_std /= np.maximum(norms, 1e-8)

        # Sort by category label to create non-stationarity
        sort_idx = np.argsort(y_raw)
        self._features = X_std[sort_idx]
        self._labels = (y_raw[sort_idx] - y_raw.mean())
