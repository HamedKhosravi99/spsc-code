...
\item {\bf Limitations}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: Limitations and extensions are consolidated in App.~\ref{app:limitations} (identification scope, $V_{\rm in}$ scope of the regret bound, CUSUM detection-delay/false-alarm trade-off, single-rank assumption of Cor.~\ref{cor:rank_adaptive}). Additional caveats are noted where they arise: the rule-of-thumb crossover $d-r\gtrsim T^{1/6}$ is qualitative, not sharp (\S\ref{sec:exp_phase}), and SPSC-Adaptive is evaluated empirically only with no formal regret guarantee.
