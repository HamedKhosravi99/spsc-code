...
\begin{abstract}
  ...
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    An $\Omega(\sqrt{cT})$ lower bound confirms the probe--subspace tradeoff is
  fundamental for explore-then-commit policies; an
...
not~$r$.
  ...
  policies (Theorem~\ref{thm:etc_lower_bound})---a class that
  includes SPSC---confirms that the probe--subspace tradeoff is
  inherent to any method that learns a subspace from probes and then
  exploits within it. The remaining gap to the
  $\widetilde{O}(c^{1/3}T^{2/3})$ upper bound arises from
  $O(\varepsilon)$ per-round bias under hard projection versus the
  $\Omega(\varepsilon^2)$ information-theoretic cost
  (Remark~\ref{rem:etc_lb_interpretation}), and closing it via
  soft-projection schemes is an explicit open problem.
...
%% ============================================================
 ...
 \section{Experiments}
  ...
  $r$-curve tracks $\sqrt{r/d}$, validating the leading-rate improvement from $d\sqrt{T}$ to $r\sqrt{T}$.
    ...
    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=0.84\textwidth]{figs/experiment1_synthetic_phase.png}
...
This work is primarily theoretical, applicable to recommendation, pricing, and control. The main open question is closing the $\Omega(\sqrt{cT})$ vs.\ $\widetilde{O}(c^{1/3}T^{2/3})$ gap via soft-projection schemes; other directions include unknown rank $r$ and time-varying transition matrices.
...
\appendix
  ...
  \tocentry{E}{app:supporting_statements}{Supporting Statements}
    ...
    \tocsubentry{E.9}{app:probe_cost_lower_bound}{Probe-Cost Lower Bound ($\Omega(\sqrt{cT})$)}
...
\paragraph{Probe-cost lower bound.}
  Theorem~\ref{thm:etc_lower_bound} (Appendix~\ref{app:probe_cost_lower_bound}) proves an $\Omega(\sqrt{cT})$ minimax lower bound for
   the probe--subspace tradeoff via a Le~Cam two-point argument. The bound applies to the class of explore-then-commit (ETC)
  policies---any method that learns a subspace from full-dimensional probes and subsequently exploits within the committed subspace.
  SPSC belongs to this class (with interleaved scheduling and an $r$-dimensional committed subspace), so the lower bound is directly
  relevant.

  The gap between $\Omega(\sqrt{cT})$ and the upper bound $\widetilde{O}(c^{1/3}T^{2/3})$ is structurally well-understood: it arises
  because hard projection onto the estimated subspace incurs $O(\varepsilon)$ per-round bias from subspace error, while the
  information-theoretic regret scales as $\Omega(\varepsilon^2)$ (Remark~\ref{rem:etc_lb_interpretation}). Concretely, the gap lives
  entirely in the probe-cost exponent ($c^{1/2}$ vs.\ $c^{1/3}$), not in the $T$-dependence, and the $\Omega(\sqrt{cT})$ lower bound
  already confirms that the $\widetilde{O}(T^{2/3})$ probe--subspace tradeoff term \emph{cannot be eliminated} by any ETC policy---it
   is inherent to the learn-then-exploit paradigm.

  Whether a tighter $\Omega(c^{1/3}T^{2/3})$ lower bound holds for the full class of adaptive policies (which may update the subspace
   continuously rather than committing) remains open. A natural path to closing the gap from the upper-bound side is a
  soft-projection scheme that achieves $O(\varepsilon^2)$ per-round cost instead of $O(\varepsilon)$; we leave this as an explicit
  open question.
...
Thm.~\ref{thm:etc_lower_bound} \newline (Probe-cost lower bound) &
  $\Omega(\sqrt{cT})$ minimax lower bound for ETC policies via Le~Cam two-point argument; confirms probe--subspace tradeoff is fundamental &
  Tightness evidence; gap to $\widetilde O(c^{1/3}T^{2/3})$ upper bound is structurally explained (Remark~\ref{rem:etc_lb_interpretation}) \\

Remark~\ref{rem:lower_bound} \newline (Probe-cost lower bound) &
  The $\Omega(\sqrt{cT})$ lower bound confirms the tradeoff is fundamental; remaining gap to $c^{1/3}T^{2/3}$ is from $O(\varepsilon)$ vs.\ $\Omega(\varepsilon^2)$ per-round cost &
  Strength: open gap is precisely characterized, not a missing result \\
...
\begin{remark}[Interpretation and the $\sqrt{cT}$ vs.\ $c^{1/3}T^{2/3}$ gap]
\label{rem:etc_lb_interpretation}
Theorem~\ref{thm:etc_lower_bound} establishes that the probe--subspace tradeoff incurs at least $\Omega(\sqrt{cT})$ regret for any ETC-type policy, confirming that the tradeoff is fundamental and not an artifact of the analysis.

The remaining gap between the lower bound $\Omega(\sqrt{cT})$ and the upper bound $\widetilde{O}(c^{1/3}T^{2/3})$ from Theorem~\ref{thm:closed_dynamic_regret} has a precise structural explanation.
The lower bound gives $\Omega(\epsilon^2)$ per-round regret from subspace misalignment (via $1-\cos\alpha \sim \alpha^2/2$), yielding $cm + \epsilon^2 T$ with $\epsilon = O(1/\sqrt{m})$, hence $cm + T/m$, optimized at $m = \sqrt{T/c}$: total $\Omega(\sqrt{cT})$.
The upper bound incurs $O(\epsilon)$ per-round bias from the hard projection step (the $2S_wR_{\cA}\varepsilon_k n_k$ term in Theorem~\ref{thm:segment_windowed_projected}), yielding $cm + \epsilon T = cm + T/\sqrt{m}$, optimized at $m = (T/c)^{2/3}$: total $O(c^{1/3}T^{2/3})$.

Whether an algorithm without hard projection can achieve $O(\epsilon^2)$ per-round overhead---and thereby close the gap to $\widetilde{O}(\sqrt{cT})$---is an open question.
The gap is in the probe-cost exponent ($c^{1/2}$ vs.\ $c^{1/3}$), not in the $T$-dependence, and is structurally well-understood.
\end{remark}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=\textwidth]{figs/experiment3_probe_ablation.png}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=0.84\textwidth]{figs/experiment_pendigits_operating_regime.png}
...
\label{app:satimage}
 ...
 \begin{figure}[!htpb]
      \centering
      \includegraphics[width=\textwidth]{figs/experiment_real_satimage_double_rings.png}
 ...
 \subsection{Robustness and necessity experiments}
  ...
  \begin{figure}[!htpb]
      \centering
      \includegraphics[width=\textwidth]{figs/experiment_robustness_abc.png}
...
\begin{figure}[!htpb]
\centering
\includegraphics[width=0.65\textwidth]{figs/experiment_rank_misspec.png}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=\textwidth]{figs/experiment2_subspace_recovery.png}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=\textwidth]{figs/experiment4_changepoint_recovery.png}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=\textwidth]{figs/experiment5_dimension_scaling.png}
...
\begin{figure}[!htpb]
  \centering
  \includegraphics[width=\linewidth]{figs/experiment6_noise_robustness.png}
...
\begin{figure}[!htpb]
  \centering
  \includegraphics[width=\linewidth]{figs/experiment7_changepoint_frequency.png}
...
\begin{figure}[!htpb]
  \centering
  \includegraphics[width=\linewidth]{figs/experiment8_drift_speed.png}
...
\begin{figure}[!htpb]
  \centering
  \includegraphics[width=\linewidth]{figs/experiment9_sota_benchmark.png}
...
\begin{figure}[!htpb]
    \centering
    \includegraphics[width=\textwidth]{figs/satimage_wincount.png}
...
\subsection{Warfarin clinical dosing benchmark}
  ...
  \begin{figure}[t]
  \centering
  \includegraphics[width=\textwidth]{figs/experiment_warfarin.png}
...
To isolate whether SPSC's gains on Warfarin arise from the \emph{learned} subspace or from
  ...
  \begin{figure}[!htpb]
  \centering
  \includegraphics[width=\textwidth]{figs/ablation_warfarin_random_subspace.png}
