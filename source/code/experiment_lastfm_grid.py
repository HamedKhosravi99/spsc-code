"""
Last.fm Fully Real Grid: d in {55, 105, 200}, r in {5, 10, 20}, 3 seeds.
Rewards = actual listening counts (log-transformed). Per-seed per-method timing.
"""
...
from environments import RealLastfmEnvironment
...
def print_cell_summary(res, d, r):
    ...
    print()
    print("=" * 110)
    print(f"  Last.fm REAL  d={d}, r={r}   ({n} seeds)   d-r={d-r}")
    print("-" * 110)
...
if __name__ == "__main__":
    print("=" * 80)
    print(f"Last.fm FULLY REAL Grid ({len(METHOD_NAMES)} methods, {N_SEEDS} seeds)")
    print(f"  d = {D_VALUES}")
    print(f"  r = {R_VALUES}")
    print(f"  SEG_SIZE={SEG_SIZE}, N_SEGMENTS={N_SEGMENTS}, "
          f"lam={LAM}, pe={PROBE_EVERY}, W={WINDOW}, c={PROBE_COST}")
    print(f"  Rewards = actual listening counts (log-transformed, centered)")
    print("=" * 80)
