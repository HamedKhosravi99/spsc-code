...
\begin{abstract}
  We characterize probe-side conditions under which a changing $r$-dimensional subspace ($r \ll d$, where $d$ is the ambient dimension) is identifiable from
  single-play scalar feedback $y_t = x_t^\top\theta_t + \varepsilon_t$ under an explicit per-probe cost $c > 0$, and prove
  matching upper and lower bounds of order
  $\widetilde{\Theta}(r\sqrt{T} + c^{1/3}T^{2/3})$ within a projected-exploitation policy class---the class containing all known algorithms for this problem.
  Our first result identifies an identification boundary for the single-play scalar-feedback model: three probe-side conditions
  ...
  Our second result is \emph{Single-Play Subspace-Calibrated Optimism} (SPSC), which achieves
  ...
  Our third result is a matching $\Omega(c^{1/3}T^{2/3})$ lower bound within the projected-exploitation class, showing
  that the probe--subspace tradeoff is sharp in that class.
  Experiments on synthetic and real-data benchmarks are consistent with the predicted phase-transition boundary $d - r \asymp T^{1/6}$ and show
  6--38\% regret reductions over ambient-space baselines in the high-dimensional low-rank regime, including clinical dosing.
  \end{abstract}
%% ============================================================
%%  1. INTRODUCTION
%% ============================================================
...
not~$r$.
  ...
  Within the projected-exploitation class---containing SPSC and all known algorithms for this problem---a matching $\Omega(c^{1/3}T^{2/3})$ lower bound
  (Theorem~\ref{thm:probe_rate_lb}) shows the tradeoff is sharp in that class
  ...
  \item \textbf{Identification boundary for single-play scalar feedback.}
  We characterize necessary-and-sufficient probe-side conditions for recovering a
  changing $r$-dimensional subspace from single-play scalar feedback within the model studied here:
  three conditions are each individually necessary
  ...
  \item \textbf{Projected-exploitation upper bound with intrinsic-rank dependence.}
  SPSC achieves $\widetilde{O}(r\sqrt{T})$ costed dynamic
  ...
  \item \textbf{Matching lower bound within the projected-exploitation class.}
  Within the projected-exploitation class---containing SPSC and
  all known algorithms for this problem---we prove a matching $\Omega(c^{1/3}T^{2/3})$ lower bound, showing that the probe--subspace
  tradeoff is sharp in that class
  ...
  and the crossover condition $d - r > C\,T^{1/6}$ provides a practical rule-of-thumb
  for finite-$T$ performance
  (\S\ref{sec:experiments}).

  \item \textbf{Theorem-relevant experiments.}
  Experiments probe the predicted phase-transition boundary, the $T^{2/3}$
  tradeoff, and the identification boundary conditions on synthetic
...
\emph{standard bandit conditions} (Assumptions~\ref{assum:latent_dynamics}--\ref{assum:action_boundedness}, standard stability and boundedness conditions in linear and nonstationary bandit analyses) and \emph{probe-side identification conditions} (Assumptions~\ref{assum:probe_realizability}--\ref{assum:probe_noise}). Within the single-play scalar-feedback model studied here, the latter are necessary for identifiability: Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c} show that dropping any single probe-side condition destroys identifiability, and Theorem~\ref{thm:restricted_span_minimax_regret} shows that violations cause $\Omega(T)$ unavoidable regret for any algorithm. Assumptions~\ref{assum:latent_dynamics}--\ref{assum:action_boundedness} are needed for validity; Assumptions~\ref{assum:probe_realizability}--\ref{assum:probe_noise} are needed for subspace recovery and the sharper $r$-dependent rate.
...
Within the model studied here, these are probe-side identification requirements, not generic modeling restrictions: each is provably necessary (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}), stated in a form that supports the identifiability argument, and the paper quantifies robustness to approximate violation (Theorem~\ref{thm:closed_dynamic_regret_general}). Their practical scope and formal necessity are discussed in Appendix~\ref{sec:discussion}.
\begin{remark}[Online variance estimation at no asymptotic cost]
  \label{rem:burnin_free}
  Dedicating the first $T^{2/3}$ probe rounds to estimating $\sigma_\varepsilon^2$ yields
  $|\hat\sigma^2 - \sigma_\varepsilon^2| = O(T^{-1/3})$, absorbed into the existing $\widetilde{O}(T^{2/3})$ tradeoff term
  (Theorem~\ref{thm:closed_dynamic_regret_general}).
  For isotropic Gaussian probes the bias is a scaled identity $\widetilde{B} \approx \frac{\delta_\sigma}{2}I_d$, shifting
  eigenvalues uniformly without corrupting eigenvectors (Proposition~\ref{prop:biased_measurement}).
  \end{remark}
...
\begin{remark}[Identification boundary: necessity and scope]
  ...
  Each probe-side condition is provably necessary within our model (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}):
  ...
  \emph{(iii)~Full-dimensional probe coverage} is strong in locked-action or safety-constrained systems, and the present paper does not claim otherwise. Within the single-play scalar-feedback model and the ambient comparator studied here, Theorem~\ref{thm:restricted_span_minimax_regret} shows that without coverage in every reward-relevant direction, sublinear costed dynamic regret is impossible over the model class. We therefore view action-constrained systems as outside the present scope.
  Whether partial coverage can support intermediate guarantees when the comparator is correspondingly restricted is an important open question.
  \end{remark}
...
\begin{equation}
S_t := \|\widehat{M}_t^{\mathrm{recent}} -
\widehat{M}_t^{\mathrm{past}}\|_{\op},
\label{eq:detector_stat}
\end{equation}
triggering subspace re-estimation when $S_t$ exceeds threshold $b$
(Eq.~\eqref{eq:detector_threshold_main}).
Theorem~\ref{thm:adaptive_regret} guarantees the same leading rate with
$O(K)$ overhead.
%% ============================================================
...
\label{sec:theory}
  ...
  the \emph{probe--subspace tradeoff} (matched within the projected-exploitation class by Theorem~\ref{thm:probe_rate_lb}),
  ...
  \emph{(ii)~Tradeoff $\widetilde{O}(T^{2/3})$:} Optimizing probe count $m_k$ balances probe cost $cm_k$ against subspace error $O(m_k^{-1/2})$; the optimum $m_k^\star \propto \ell_k^{2/3}$ yields $T^{2/3}$. Within the projected-exploitation class, Theorem~\ref{thm:probe_rate_lb} establishes a matching lower bound at the same rate.
  ...
  Overestimation ($\hat r > r$) costs a factor $\hat r/r$ in the statistical
  term only; underestimation adds a penalty proportional to signal energy in the
  missed directions. Full analysis in Appendix~\ref{sec:discussion}.
  \end{corollary}
...
\end{equation}
  ...
  Two lower bounds establish that the probe--subspace tradeoff is fundamental.
  First, Theorem~\ref{thm:etc_lower_bound}
  (Appendix~\ref{app:probe_cost_lower_bound}) proves $\Omega(\sqrt{cT})$ for
  all explore-then-commit policies via a Le~Cam two-point argument,
  establishing the tradeoff as an information-theoretic barrier.
  The following sharper result applies within the class of algorithms that
  commit to an estimated subspace:
  ...
  The two lower bounds have complementary scopes: $\Omega(\sqrt{cT})$ holds
  unconditionally for ETC policies, while $\Omega(c^{1/3}T^{2/3})$ is
  specific to the projected-exploitation class.
  The gap opens a precise question (Remark~\ref{rem:eps_gap_resolved}):
  can non-projecting algorithms achieve $O(\sqrt{cT})$?
 ...
 \section{Experiments}
  ...
  The crossover provides a practical rule-of-thumb: use SPSC when $d - r \gtrsim T^{1/6}$, ambient LinUCB otherwise.
    ...
    \begin{figure}[!htpb]
        ...
        The empirical crossover boundary closely tracks $d{-}r = T^{1/6}$ (dashed) at finite $T{=}5{,}000$, consistent with
  ...
  \paragraph{Experiment 4: Identification boundary probes.}
  Three ablations
  (Appendix~\ref{app:robustness_necessity}) probe the necessity and robustness of each identification condition: $5\times$ variance
  misspecification $\to$ ${<}5\%$ regret change; $\epsilon_\times{=}1$ $\to$ ${<}1\%$
  regret change; restricted coverage $\to$ $\Omega(T)$ blow-up, consistent with the lower bound in
  Theorem~\ref{thm:restricted_span_minimax_regret}. The first two conditions degrade
  gracefully as predicted by Theorem~\ref{thm:closed_dynamic_regret_general}.

  \paragraph{Experiment 5: Probe-rate tradeoff (Theorem~\ref{thm:probe_rate_lb}).}
  Sweeping the probe period over $\{5,\ldots,300\}$ produces a U-shaped regret curve
  (Figure~\ref{fig:probe-ablation}, Appendix~\ref{app:probe_ablation}):
  sparse probing yields poor subspace estimates; excessive probing wastes exploitation rounds.
  The optimum at period ${\approx}20$--$30$ is consistent with the $m_k^\star \propto \ell_k^{2/3}$ allocation
  predicted by Theorem~\ref{thm:closed_dynamic_regret}, and the monotone relationship between
  late-segment subspace error and regret directly links SPSC's gains to subspace recovery quality.

\vspace{-2pt}
\section{Conclusion}
\label{sec:conclusion}

We characterize probe-side conditions under which a changing low-rank subspace is identifiable from single-play
   scalar feedback, and show that these conditions yield a rank-dependent costed dynamic regret upper bound of order $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ for SPSC.
Within the projected-exploitation class, Theorem~\ref{thm:probe_rate_lb} provides a matching $\Omega(c^{1/3}T^{2/3})$ lower bound, so the probe--subspace tradeoff is sharp in that class.
An adaptive variant removes oracle segment boundaries without changing the leading rate.
Empirically, the observed crossover behavior and 6--38\% regret reductions are consistent with the theorem's predicted regime separation.
