...
\begin{abstract}
  We resolve when a changing $r$-dimensional subspace ($r \ll d$, where $d$ is the ambient dimension) is identifiable from
  single-play scalar feedback $y_t = x_t^\top\theta_t + \varepsilon_t$ under a per-probe cost $c > 0$ explicitly accounted
  in the regret criterion, and prove that the optimal costed dynamic regret is
  $\widetilde{\Theta}(r\sqrt{T} + c^{1/3}T^{2/3})$ for the class of projected-exploitation
  policies---the class containing every known algorithm for this problem.
  The first result is a sharp identification boundary: three probe-side conditions
  (sub-Gaussian noise with online variance estimation, bounded state--noise coupling, and full-dimensional probe coverage) are
  each individually necessary---with formal impossibility results for every violation
  (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c})---and jointly sufficient for subspace recovery via a lifted matrix
  operator (Propositions~\ref{prop:quad}--\ref{prop:lifted_probe_unbiased}).
  The second result is our algorithm \emph{Single-Play Subspace-Calibrated Optimism} (SPSC), the first to achieve
  $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ costed dynamic regret under piecewise-stationary
  low-rank rewards---replacing the ambient $\widetilde{O}(d\sqrt{T})$ dependence with intrinsic rank~$r$---with an
  adaptive variant that detects change points without oracle boundaries at no asymptotic cost.
  The third result is a tight $\Omega(c^{1/3}T^{2/3})$ lower bound for projected-exploitation policies, establishing
  that the probe--subspace tradeoff in SPSC is unimprovable within this class.
  Experiments validate the predicted phase-transition boundary $d - r \asymp T^{1/6}$: SPSC achieves
  6--38\% regret reductions over ambient-space baselines in the high-dimensional low-rank regime on synthetic and real-data
  benchmarks including clinical dosing.
  \end{abstract}
%% ============================================================
%%  1. INTRODUCTION
%% ============================================================
...
not~$r$.
  ...
  enables subspace recovery, but probing is priced explicitly: each probe round incurs cost $c$.
  ...
  The optimal allocation $m^\star = \Theta(T^{2/3})$ balances these exactly,
  yielding a $\widetilde{O}(T^{2/3})$ tradeoff term that Theorem~\ref{thm:probe_rate_lb} proves is unavoidable for all projected-exploitation policies.
  \end{enumerate}
  ...
  \emph{Single-Play Subspace-Calibrated Optimism} (SPSC) interleaves probing with
  ...
  policies (Theorem~\ref{thm:etc_lower_bound}) establishes the
  probe--subspace tradeoff as an information-theoretic barrier.
  A tight $\Omega(c^{1/3}T^{2/3})$ lower bound
  (Theorem~\ref{thm:probe_rate_lb}) matches the upper bound
  for the class of projected-exploitation
  policies---the class containing SPSC and all known algorithms for this problem---confirming the tradeoff is unimprovable within this class
  ...
  \item \textbf{Sharp identification boundary (first complete characterization).}
  We prove the first necessary-and-sufficient conditions for recovering a
  changing $r$-dimensional subspace from single-play scalar feedback:
  three probe-side conditions are each individually necessary
  (with formal impossibility for every violation) and jointly sufficient
  for subspace recovery via a lifted matrix operator (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c},
  Propositions~\ref{prop:quad}--\ref{prop:lifted_probe_unbiased}).

  \item \textbf{Optimal algorithm.}
  SPSC is the first algorithm achieving $\widetilde{O}(r\sqrt{T})$ costed dynamic
  regret under piecewise-stationary low-rank rewards---replacing the ambient
  $\widetilde{O}(d\sqrt{T})$ dependence with intrinsic rank $r$---with an adaptive
  variant that detects change points without oracle boundaries at no asymptotic cost
  (Theorems~\ref{thm:closed_dynamic_regret},~\ref{thm:adaptive_regret}).

  \item \textbf{Tight lower bounds.}
  We prove matching $\Omega(c^{1/3}T^{2/3})$ lower bounds for
  projected-exploitation policies---the class containing SPSC and
  all known algorithms for this problem---establishing that the probe--subspace
  tradeoff is \emph{unimprovable} within this class
  (Theorem~\ref{thm:probe_rate_lb}).

  \item \textbf{Robustness under misspecification.}
  The leading rate $\widetilde{O}(r\sqrt{T} + T^{2/3})$ is preserved under
  $T^{-1/3}$-level probe misspecification (Theorem~\ref{thm:closed_dynamic_regret_general}),
  and a deployable crossover criterion $d - r > C\,T^{1/6}$ predicts
  finite-$T$ performance
  (\S\ref{sec:experiments}).

  \item \textbf{Theorem-relevant experiments.}
  Experiments validate the predicted phase-transition boundary, the tight $T^{2/3}$
  probe--subspace tradeoff, and the identification boundary conditions on synthetic
  and real-data benchmarks including clinical dosing, with 6--38\% regret reductions
  over ambient-space baselines (\S\ref{sec:experiments}).
  \end{enumerate}
...
\paragraph{The gap this work fills.}
...
SPSC is the \emph{first algorithm} satisfying all three, and our identification boundary (\S\ref{sec:identification}) is the \emph{first formal characterization} of when (i)--(iii) are simultaneously achievable from single-play scalar feedback.
...
Our assumptions divide into two categories:
\emph{standard bandit conditions} (Assumptions~\ref{assum:latent_dynamics}--\ref{assum:action_boundedness}, satisfied by all prior work in this space) and \emph{probe-side identification conditions} (Assumptions~\ref{assum:probe_realizability}--\ref{assum:probe_noise}). The latter are not modeling choices but \emph{information-theoretic requirements}: Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c} show that dropping any single probe-side condition destroys identifiability, and Theorem~\ref{thm:restricted_span_minimax_regret} shows that violations cause $\Omega(T)$ unavoidable regret for any algorithm. Assumptions~\ref{assum:latent_dynamics}--\ref{assum:action_boundedness} are needed for validity; Assumptions~\ref{assum:probe_realizability}--\ref{assum:probe_noise} are needed for subspace recovery and the sharper $r$-dependent rate.
...
These conditions are probe-side identification requirements, not generic modeling restrictions: each is provably necessary (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}), stated in its weakest usable form, and robust to approximate violation (Theorem~\ref{thm:closed_dynamic_regret_general}). Their practical scope and formal necessity are discussed in Appendix~\ref{sec:discussion}.
...
\begin{remark}[Identification boundary: necessity and practical scope]
  \label{rem:necessity_compact}
  Each probe-side condition is provably necessary (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}); all three are stated in their weakest usable form:
  \emph{(i)~Noise variance} is unknown and estimated online; burn-in cost $O(T^{2/3})$ is absorbed into the existing tight tradeoff term
  ...
  \emph{(iii)~Full-dimensional probe coverage} is an information-theoretic requirement, not an artifact of our analysis: Theorem~\ref{thm:restricted_span_minimax_regret} establishes that \emph{no algorithm} can achieve sublinear costed dynamic regret without coverage in every reward-relevant direction. The condition $\supp(Q) \subseteq \cA_t$ is satisfied by any system that reserves a fraction of rounds for randomized exploration, including recommendation platforms (traffic splitting), clinical trials (randomized arms), and adaptive control (random perturbations for system identification).
  \end{remark}
...
\label{sec:theory}
  ...
  This section states the main regret guarantees, which form a three-level hierarchy: Theorem~\ref{thm:closed_dynamic_regret} proves the optimal rate under exact conditions; Theorem~\ref{thm:closed_dynamic_regret_general} proves the same leading rate is preserved under approximate conditions;
  Theorem~\ref{thm:adaptive_regret} removes oracle segment boundaries at no asymptotic cost. The underlying decomposition is in Theorem~\ref{thm:full_horizon_windowed} (Appendix~\ref{app:all_proofs}).
Proofs are in Appendix~\ref{app:all_proofs} (\S\ref{proof:full_horizon_windowed}, \S\ref{proof:closed_dynamic_regret},
  ...
  \begin{theorem}[Optimal costed dynamic regret]
  \label{thm:closed_dynamic_regret}
  Under Assumptions~\ref{assum:latent_dynamics}--\ref{assum:probe_noise}, with optimal probe allocation
  ...
  the \emph{tight probe--subspace tradeoff} (matched by Theorem~\ref{thm:probe_rate_lb}),
  ...
  \emph{(i)~Projected exploitation $\widetilde{O}(r\sqrt{T})$:} After subspace recovery, SPSC runs windowed ridge-UCB in $\R^r$ with confidence radius $\beta^{(r,W)} \propto \sqrt{r}$ (vs.\ $\sqrt{d}$ for ambient methods). The elliptical potential lemma applied in $r$ dimensions gives $\widetilde{O}(r\sqrt{T})$ statistical regret.
  \emph{(ii)~Tight tradeoff $\widetilde{O}(T^{2/3})$:} Optimizing probe count $m_k$ balances probe cost $cm_k$ against subspace error $O(m_k^{-1/2})$; the optimum $m_k^\star \propto \ell_k^{2/3}$ yields $T^{2/3}$. Theorem~\ref{thm:probe_rate_lb} proves this rate is a lower bound for all projected-exploitation policies, confirming tightness.
  \emph{(iii)~Drift $O(V)$:} Within-segment parameter variation contributes linearly and is irreducible. Full proof in Appendix~\ref{app:all_proofs}, \S\ref{proof:closed_dynamic_regret}.
  \end{proof}

  \emph{Window size in theory vs.\ practice.} The choice $W^\star = 1$ minimizes the worst-case bound under an adversarial within-segment variation argument. Under the autocorrelated LDS dynamics of Assumption~\ref{assum:latent_dynamics}, however, the effective local variation is much smaller than the worst case, so larger windows reduce statistical error without inflating the drift term. All experiments use $W \in [100, 400]$; the probe-rate ablation in Appendix~\ref{app:probe_ablation} confirms a U-shaped tradeoff with the optimum near $W = 100$.
...
%% ============================================================
 ...
 \section{Experiments}
  ...
  We evaluate SPSC on synthetic and real-data benchmarks, with each experiment designed to validate a specific theorem-relevant phenomenon.
  ...
  \paragraph{Choice of baselines.} The primary competitors are nonstationary linear bandits in the ambient $d$-dimensional space (D-LinUCB, SW-LinUCB, Restart-LinUCB), which are the state of the art for the setting SPSC targets: changing reward parameters under bounded scalar feedback. Oracle-LinUCB (with the true subspace) provides a performance ceiling. We do \emph{not} include restarted versions of recent stationary low-rank methods (e.g., \citet{pmlr-v235-jedra24a, pmlr-v235-jang24e}) because they assume richer feedback models or stationary representations, and a naive restart wrapper would not be apples-to-apples in our single-play scalar-feedback setting; Appendix~\ref{app:related_table} discusses this in detail.
  ...
  precisely tracks the predicted $d{-}r = T^{1/6}$ threshold from Eq.~\eqref{eq:regime_condition}: in the high-dimensional low-rank regime ($d \ge 45$), SPSC achieves $13$--$34\%$ regret reductions, and every
  $r$-curve tracks $\sqrt{r/d}$, validating the leading-rate improvement from $d\sqrt{T}$ to $r\sqrt{T}$.
  The crossover serves as a deployable criterion: use SPSC when $d - r \gtrsim T^{1/6}$, ambient LinUCB otherwise.

    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=0.84\textwidth]{experiment1_synthetic_phase.png}
        \caption{\textbf{Empirical validation of the phase-transition boundary
        (Theorems~\ref{thm:closed_dynamic_regret} and~\ref{thm:ambient_regret}).}
        \textbf{(a)}~SPSC/LinUCB cumulative regret ratio over the $(d,r)$ grid.
        The empirical crossover boundary tracks $d{-}r = T^{1/6}$ (dashed) precisely at finite $T{=}5{,}000$, confirming
        Eq.~\eqref{eq:regime_condition}.
        \textbf{(b)}~Ratio vs.\ $d$ (log-scale) for each latent rank~$r$;
        empirical curves track the $\sqrt{r/d}$ theoretical prediction (dotted).}
  ...
  The crossover criterion $d - r > C\,T^{1/6}$ correctly predicts the regime where SPSC yields gains, confirming the theory is predictive at finite~$T$.
  ...
  \textbf{0.76} \\
    ...
    (subspace error $\approx 0.96$ to $1.00$), directly validating Theorem~\ref{thm:closed_dynamic_regret_general}: the penalty from approximate
  ...
  \paragraph{Experiment 4: Identification boundary validation.}
  Three experiments validate the necessity and robustness of the identification conditions (Appendix~\ref{app:robustness_necessity}): $5\times$ variance
  misspecification changes regret by $<5\%$; cross-correlation $\epsilon_\times{=}1$ changes regret by $<1\%$; restricting coverage
  causes $\Omega(T)$ blow-up as predicted by the information-theoretic lower bound. These confirm not just performance but the \emph{mechanism}: the first two conditions degrade
  gracefully (Theorem~\ref{thm:closed_dynamic_regret_general}), while the third is an information-theoretic requirement
  (Theorem~\ref{thm:restricted_span_minimax_regret}). The probe-rate ablation (Appendix~\ref{app:probe_ablation}) validates the predicted $T^{2/3}$ tradeoff curve, confirming tightness of Theorem~\ref{thm:probe_rate_lb}.
...
We resolve the probe-side identification boundary for recovering a changing low-rank subspace from single-play
   scalar feedback: three conditions are each individually necessary and jointly sufficient, constituting the first complete characterization for this problem.
SPSC achieves $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ costed dynamic regret---the first algorithm to replace the ambient $d\sqrt{T}$ dependence with intrinsic rank $r$---and Theorem~\ref{thm:probe_rate_lb} establishes that the $T^{2/3}$ tradeoff is tight for all projected-exploitation policies.
An adaptive variant removes oracle segment boundaries at no asymptotic cost.
Experiments validate the predicted phase-transition boundary and confirm 6--38\% regret reductions over all nonstationary baselines in the high-dimensional low-rank regime.

\paragraph{Limitations and open problems.}
Three directions remain open.
\emph{(i)~Non-projecting algorithms:} Whether algorithms that operate in the full $d$-dimensional ambient space can achieve $O(\sqrt{cT})$ total regret by exploiting $O(\varepsilon^2)$ per-round subspace misalignment cost (vs.\ the $O(\varepsilon)$ cost inherent to projection) is the principal open question identified by the gap between Theorems~\ref{thm:etc_lower_bound} and~\ref{thm:probe_rate_lb}.
\emph{(ii)~Unknown rank:} The eigenvalue thresholding rule (Eq.~\eqref{eq:rank_selection}) provably recovers the true rank when the eigengap is large; the case of a gradually decaying spectrum without a clear gap requires new techniques.
\emph{(iii)~Time-varying dynamics:} Extending SPSC to segment-varying transition matrices $A_k$ that are themselves nonstationary would broaden applicability to rapidly changing systems.
...
\paragraph{Probe-cost lower bounds.}
  ...
  Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes a tight matching $\Omega(c^{1/3}T^{2/3})$ lower bound for projected-exploitation policies (Definition~\ref{def:hard_projection_policy_class})---the class containing SPSC and all known algorithms for this problem---via a fixed two-point lower-bound family satisfying Assumption~\ref{assum:probe_realizability}.
  The $\widetilde{O}(c^{1/3}T^{2/3})$ probe--subspace tradeoff term in Theorem~\ref{thm:closed_dynamic_regret} is therefore tight for this policy class.

  \emph{Scope summary.} The two lower bounds have complementary scopes: $\Omega(\sqrt{cT})$ is the unconditional ETC-style barrier that holds for \emph{all} admissible policies, while $\Omega(c^{1/3}T^{2/3})$ is tight specifically for projected-exploitation policies. The gap between these bounds opens a precise new question: can non-projecting algorithms that operate in the full ambient space achieve $O(\sqrt{cT})$ total regret?
...
\midrule
 SPSC (ours) & This work & $\widetilde O(r\sqrt{T} + c^{1/3}T^{2/3})$ & Nonstationary; learns $B$ via probing \\
\bottomrule
\end{tabular}
...
\begin{definition}[Hard-projection score-maximizing policy class]
\label{def:hard_projection_policy_class}
A policy belongs to the \emph{hard-projection score-maximizing class} $\Pi_{\mathrm{HP}}$ if, after its probe rounds, it forms an estimated rank-$r$ projector $\widehat{P}_t$ and a score vector $\vartheta_t \in \range(\widehat{P}_t)$, and on each exploitation round chooses
\[
...
\begin{theorem}[Matching $\Omega(c^{1/3}T^{2/3})$ lower bound for hard-projection score-maximizing policies]
\label{thm:probe_rate_lb}
Under Assumptions~\ref{assum:latent_dynamics}--\ref{assum:probe_noise} with $d \ge 2r$, there exists a constant $c_4 > 0$ such that for all sufficiently large $T$,
\[
...
\ge c_4\, c^{1/3} T^{2/3},
\]
where $\Pi_{\mathrm{HP}}$ is the hard-projection score-maximizing class from Definition~\ref{def:hard_projection_policy_class}, and $\mathcal{M}_{\mathrm{HP}}$ is the fixed two-point lower-bound family defined below.
\end{theorem}
