...
\begin{abstract}
  ...
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    An $\Omega(c^{1/3}T^{2/3})$ lower bound for projected-exploitation policies matches the upper bound, confirming the probe--subspace tradeoff is tight; an
...
not~$r$.
  ...
  (Theorem~\ref{thm:probe_rate_lb}) matches the upper bound for
  all projected-exploitation policies---a class that includes SPSC
  and all known algorithms for this problem---closing the gap
...
Under Assumption~\ref{assum:probe_realizability}, the operator $\mathcal K(M):=\E[(u^\top M u)\,uu^\top], \qquad u\sim Q,$ is invertible on $\R_{\mathrm{sym}}^{d\times d}$, with $\|\mathcal K^{-1}\|_{\op} \le C_{\mathcal K}(\rho, L, d)$.
\end{lemma}
...
where $\|\widetilde B_t\|_2 \le |\delta_\sigma|\,C_{\mathcal K}\,L^2 + 2\,C_{\mathcal K}\,L^2\,\epsilon_\times$.
...
Proofs are in Appendix~\ref{app:all_proofs} (\S\ref{proof:full_horizon_windowed}, \S\ref{proof:closed_dynamic_regret},
  ...
  K}L^2\epsilon_\times \le \lambda_{\min}/4$, then with probability at least $1-\delta$,
  \begin{equation}
...
This work is primarily theoretical, applicable to recommendation, pricing, and control. The $\Omega(c^{1/3}T^{2/3})$ lower bound (Theorem~\ref{thm:probe_rate_lb}) closes the gap for projected-exploitation policies; the remaining open question is whether non-projecting algorithms can achieve $O(\sqrt{cT})$. Other directions include unknown rank $r$ and time-varying transition matrices.
...
\label{sec:discussion}
 ...
 Concretely, we outline three deployable instantiations:
  ...
  action
    ...
    guarantees is an open question; the lifted operator approach requires invertibility of $\mathcal{K}$, which fails
    without full rank $\E[uu^\top]$.
...
\paragraph{Probe-cost lower bounds.}
  ...
  Building on this, Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes a matching $\Omega(c^{1/3}T^{2/3})$ lower bound for all projected-exploitation policies---any method that estimates a subspace and exploits within it, including SPSC.
  This closes the gap between the upper and lower bounds for this policy class: the $\widetilde{O}(c^{1/3}T^{2/3})$ probe--subspace tradeoff term in Theorem~\ref{thm:closed_dynamic_regret} is tight.
...
Lemma~\ref{lem:K_invertible} \newline ($\mathcal K$ invertibility) &
  Probe moment operator $\mathcal K(M)\!=\!\E[(u^\top\! Mu)\,uu^\top]$ is invertible on $\R_{\mathrm{sym}}^{d\times d}$ &
  Lifts scalar data to matrix estimates. Dep: Assump.~\ref{assum:probe_realizability} \\
...
Proposition~\ref{prop:lifted_probe_unbiased} \newline (Unbiased lifted probe) &
  $\E[G_t\mid\cH_{t-1}]\!=\!\widetilde M_t$: each lifted sample unbiased for $\widetilde M_t$ &
...
Thm.~\ref{thm:probe_rate_lb} \newline (Matching $\Omega(c^{1/3}T^{2/3})$) &
  Matches upper bound for all projected-exploitation policies &
  Closes the gap (Remark~\ref{rem:eps_gap_resolved}) \\
...
&
Ensures that no potentially active subspace direction is invisible to probing, and guarantees invertibility of the probe operator used to lift quadratic measurements into matrix estimates.
&
...
Proof in \S\ref{proof:biased_measurement}.
 \begin{proposition}[Biased measurement under bounded cross-correlation]
  ...
  \]
  where the bias satisfies $\|\widetilde B_\times^{(t)}\|_2 \le 2\,C_{\mathcal K}\,L^2\,\epsilon_\times$.
  \end{proposition}
...
\subsection{Minimax Optimality of the $1/\sqrt{m}$ Subspace Recovery Rate}
...
\begin{proof}
The proof applies Fano's inequality to a packing of the Grassmannian manifold $\mathrm{Gr}(r, d)$.

\emph{Grassmannian packing.}
By standard volumetric arguments~\citep{edelman1998geometry, pajor1998metric}, for any $\delta \in (0, 1/2)$ there exists a $2\delta$-packing $\{V_1, \ldots, V_N\} \subset \mathrm{Gr}(r, d)$ with
$\|P_{V_i} - P_{V_j}\|_{\op} \ge 2\delta$ for all $i \neq j$ and $\log N \ge c_0\, r(d - r) \log(1/\delta)$,
where $c_0 > 0$ is universal.

\emph{Instance construction.}
For each $j$, let $B_j$ be an orthonormal basis of $V_j$.  Define instance $\nu_j$: one segment ($K = 1$), $B^\star = B_j$, $A_1 = 0$ (i.i.d.\ latent state $w_t = \eta_{t-1}$ with $\eta_{t-1} \sim \mathcal{N}(0, \lambda_{\min} I_r)$), probes $u_t \sim \mathcal{N}(0, I_d)$ truncated to $\|u\| \le L$, and $\varepsilon_t \sim \mathcal{N}(0, \sigma_\varepsilon^2)$.  The predictable second moment is $\widetilde{M}_t = \lambda_{\min} P_{V_j}$, constant across time.

\emph{Pairwise KL divergence.}
Conditional on $u_t = u$, $y_t \mid u \sim \mathcal{N}(0, \lambda_{\min} u^\top P_{V_i} u + \sigma_\varepsilon^2)$ under $\nu_i$.
Using $|\sigma_i^2(u) - \sigma_j^2(u)| \le \lambda_{\min} L^2 \|P_{V_i} - P_{V_j}\|_{\op}$ and the KL bound $\frac{1}{2}(x - 1 - \log x) \le (x-1)^2$:
\[
...
\emph{Fano's inequality and balancing.}
By the generalized Fano inequality, if $\max_{i \neq j} \KL(\nu_i^m \| \nu_j^m) \le \frac{1}{2}\log N$, then $\inf_{\widehat{P}} \max_j \E_{\nu_j}[\|\widehat{P} - P_{V_j}\|_{\op}] \ge \delta/2$.
Setting $\delta = c_2/\sqrt{m}$ with $c_2 = \sqrt{c_0(d-r)/(256\, C_1)}$ satisfies the Fano condition for $m \ge e$, yielding
$\inf_{\widehat{P}} \sup_{\nu} \E_\nu[\|\widehat{P} - P^\star\|_{\op}] \ge c_2/(4\sqrt{m})$.
\end{proof}

\begin{remark}
The upper bound from Davis--Kahan (Theorem~\ref{thm:projector_concentration_summary}) gives $\|\widehat{P}_k - P_k^\star\|_{\op} \le C_{\mathrm{sub}}\sqrt{\log(d/\delta)/m_k}$, matching the lower bound up to a $\sqrt{\log d}$ factor.
\end{remark}

\subsection{Matching $\Omega(c^{1/3}T^{2/3})$ Lower Bound for Projected Exploitation}
...
\begin{proof}
Fix any policy $\pi$ with $m$ probe rounds.  Probe cost is at least $cm$.
By Theorem~\ref{thm:minimax_subspace_recovery}, $\sup_\nu \E[\|\widehat{P} - P^\star\|_{\op}] \ge c_1/\sqrt{m}$.
Subspace misalignment by angle $\alpha \sim \varepsilon$ incurs per-round regret $\Omega(\varepsilon^2 S_w L_x)$ (via $1 - \cos\alpha \sim \alpha^2/2$), giving exploitation regret $\ge c_1^2 S_w L_x (T-m)/(4m)$.
Setting $\beta := c_1^2 S_w L_x/4$: total $\ge cm + \beta T/m \ge 2\sqrt{c\beta T}$ by AM--GM.
\end{proof}
...
Projected exploitation with $\|\widehat{P} - P^\star\|_{\op} = \varepsilon$ incurs per-round regret $\ge \mu\varepsilon$.  This $O(\varepsilon)$ scaling is irreducible: for any action $x$ with $\|x\| \le L_x$,
$|x^\top(\widehat{P} - P^\star)\theta_t| \le L_x S_w \varepsilon$,
and the policy has no access to directions in $\range(P^\star) \setminus \range(\widehat{P})$.

\emph{Substitution of minimax rate.}
By Theorem~\ref{thm:minimax_subspace_recovery}, after $m$ probes, $\sup_\nu \E[\|\widehat{P} - P^\star\|_{\op}] \ge c_1/\sqrt{m}$.  The exploitation regret over $T - m$ rounds is at least $\mu c_1(T-m)/\sqrt{m}$.

\emph{Optimization over $m$.}
Setting $\gamma := \mu c_1$, total costed regret $\ge cm + \gamma T/(2\sqrt{m})$ for $m \le T/2$.
Setting $c - \gamma T/(4m^{3/2}) = 0$ gives $m^\star = (\gamma T/(4c))^{2/3}$.
Substituting:
\[
...
The condition of Theorem~\ref{thm:probe_rate_lb} is satisfied by any policy that performs projected exploitation---including SPSC and all known algorithms for this problem---because hard projection onto a misaligned subspace creates an irreducible $O(\varepsilon)$ bias.
...
SPSC achieves $\widetilde{O}(\sqrt{rT}) + \widetilde{O}(c^{1/3}T^{2/3}) + O(V)$ via projected exploitation with $m \asymp (T/c)^{2/3}$ probes per segment.  Theorem~\ref{thm:probe_rate_lb} shows that the $c^{1/3}T^{2/3}$ term is tight for any policy using projected exploitation.
\end{remark}
...
where the first term is the probe cost and the second is the subspace-error--induced regret.  The total regret $\sum_{k=1}^K R_k(m_k)$ is minimized by
\begin{equation}
...
\begin{proof}
For fixed $k$, taking the derivative $R_k'(m_k) = c - \gamma\ell_k/(2m_k^{3/2}) = 0$ yields $m_k^\star = (\gamma\ell_k/(2c))^{2/3}$.  Substituting:
\[
...
Under both sources, the lifted estimator satisfies $\E[G_t \mid \cH_{t-1}] = \widetilde M_t + \widetilde B_t$, where $\|\widetilde B_t\|_2 \le C_{\mathcal K}|\delta_\sigma|L^2 + 2C_{\mathcal K}L^2\epsilon_\times =: B_{\mathrm{total}}$.
