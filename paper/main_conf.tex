...
\begin{abstract}
  ...
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    Under the natural condition that projected exploitation incurs $\Omega(\varepsilon)$ per-round regret from subspace error $\varepsilon$, an $\Omega(c^{1/3}T^{2/3})$ lower bound matches the upper bound for all projected-exploitation policies, confirming the probe--subspace tradeoff is tight for this class; an
...
not~$r$.
  ...
  for all projected-exploitation policies---a class that includes
  SPSC and all known algorithms for this problem
...
\paragraph{Probe-cost lower bounds.}
  ...
  Building on this, Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes a matching $\Omega(c^{1/3}T^{2/3})$ lower bound for all projected-exploitation policies---any method that estimates a subspace and exploits within it, including SPSC---under the condition that projected exploitation with subspace error $\varepsilon$ incurs $\Omega(\varepsilon)$ per-round regret.
  This condition is satisfied by hard projection (which creates an irreducible $O(\varepsilon)$ bias), confirming that the $\widetilde{O}(c^{1/3}T^{2/3})$ probe--subspace tradeoff term in Theorem~\ref{thm:closed_dynamic_regret} is tight for this policy class.
