...
\begin{abstract}
  ...
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    For the class of hard-projection score-maximizing policies---capturing the hard-projection mechanism underlying SPSC-style projected exploitation---a matching unconditional $\Omega(c^{1/3}T^{2/3})$ lower bound confirms the probe--subspace tradeoff is tight for this class; an
...
not~$r$.
  ...
  unconditionally for the class of hard-projection score-maximizing
  policies, a class capturing the hard-projection mechanism
  underlying SPSC-style projected exploitation
...
%% ============================================================
 ...
 \section{Experiments}
  ...
  \paragraph{Choice of baselines.} The primary competitors are nonstationary linear bandits in the ambient $d$-dimensional space (D-LinUCB, SW-LinUCB, Restart-LinUCB), which are the state of the art for the setting SPSC targets: changing reward parameters under bounded scalar feedback. Oracle-LinUCB (with the true subspace) provides a performance ceiling. We do \emph{not} include restarted versions of recent stationary low-rank methods (e.g., \citet{pmlr-v235-jedra24a, pmlr-v235-jang24e}) because they assume richer feedback models (bilinear/full-vector observations) or stationary representations, and a naive restart wrapper would not be apples-to-apples in our single-play scalar-feedback setting; Appendix~\ref{app:related_table} discusses this in detail.
...
This work is primarily theoretical, applicable to recommendation, pricing, and control. The $\Omega(c^{1/3}T^{2/3})$ lower bound (Theorem~\ref{thm:probe_rate_lb}) unconditionally matches the upper bound for hard-projection score-maximizing policies; the remaining open question is whether broader policy classes or ambient-space algorithms can achieve $O(\sqrt{cT})$. Other directions include unknown rank $r$ and time-varying transition matrices.
...
\paragraph{Probe-cost lower bounds.}
  ...
  Building on this, Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes an unconditional matching $\Omega(c^{1/3}T^{2/3})$ lower bound for hard-projection score-maximizing policies (Definition~\ref{def:hard_projection_policy_class})---a class capturing the hard-projection mechanism underlying SPSC-style projected exploitation---via a fixed two-point lower-bound family satisfying Assumption~\ref{assum:probe_realizability}.
...
Whether a broader projected-exploitation class, or algorithms operating in the full ambient space, can achieve $O(\sqrt{cT})$ total regret remains open.
\end{remark}
