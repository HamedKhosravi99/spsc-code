...
\begin{abstract}
  ...
  Low-rank structure is pervasive in high-dimensional sequential decision-making, yet exploiting it under nonstationarity remains
  open: existing low-rank bandit methods assume a fixed representation, while nonstationary algorithms pay the full ambient cost
  $\widetilde{O}(d\sqrt{T})$.
  We study the fundamental question of \emph{when and how} a changing $r$-dimensional subspace ($r \ll d$) can be identified from
  single-play scalar feedback $y_t = x_t^\top\theta_t + \varepsilon_t$.
  Under the standing latent excitation assumptions, we characterize the \emph{probe side identification boundary}: three
  conditions (sub Gaussian noise with unknown variance estimated online, bounded state noise coupling, and full dimensional
  probe coverage) are each sufficient and essentially necessary for subspace recovery.
  Under these conditions, each probe yields a quadratic measurement of the predictable second moment $\widetilde{M}_t$ with controlled bias,
  enabling recovery via a lifted matrix operator; complementary impossibility results show that removing \emph{any one} condition
  destroys identifiability, while approximate violations degrade gracefully.
  Building on this characterization, we propose \emph{Single-Play Subspace-Calibrated Optimism} (SPSC), achieving costed dynamic
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    Under the natural condition that projected exploitation incurs $\Omega(\varepsilon)$ per-round regret from subspace error $\varepsilon$---a condition satisfied by hard-projection policies including SPSC---a matching $\Omega(c^{1/3}T^{2/3})$ lower bound confirms the probe--subspace tradeoff is tight; an
  adaptive variant removes the need for oracle segment boundaries. Experiments confirm the predicted phase transition: SPSC achieves regret reductions over ambient-space baselines when $d - r \gg
  T^{1/6}$, and incurs higher cost when $d - r \lesssim T^{1/6}$, matching theory on synthetic and real-data benchmarks.
  \end{abstract}
%% ============================================================
%%  1. INTRODUCTION
%% ============================================================
...
not~$r$.
  ...
  The observation $y_t = x_t^\top B_k^\star w_t + \varepsilon_t$ is
  a scalar projection of a rank-$r$ signal.
  Recovering the $r$-dimensional column space $\range(B_k^\star)$
  from such projections requires extracting second-order information:
  the predictable second moment
  $\widetilde{M}_t := \E[\theta_t\theta_t^\top \mid \cH_{t-1}]$
  (the conditional covariance of the reward parameter given the
  learner's history $\cH_{t-1}$) satisfies
  $\range(\widetilde{M}_t) \subseteq \range(B_k^\star)$, but a
  single observation $y_t$ does not directly reveal $\widetilde{M}_t$.
  \item \textbf{Probe--subspace cost tradeoff.}
  Probing with random isotropic actions $u_t$ drawn from a known
  distribution $Q$ (satisfying $\E[u_tu_t^\top] \succeq \rho I_d$
  for a constant $\rho > 0$, ensuring coverage in every direction)
  enables subspace recovery, but each probe round forfeits
  exploitation and incurs cost $c$.
  With $m$ probes per segment, the subspace estimation error is
  $\varepsilon = O(1/\sqrt{m})$ and the exploitation regret from
  mismatch is $O(\varepsilon T)$, while the probe cost is $O(cm)$.
  Optimizing gives $m^\star = \Theta(T^{2/3})$ and a tradeoff term
  of $\widetilde{O}(T^{2/3})$.
  \end{enumerate}

  We resolve both difficulties simultaneously. Under three structural
  ...
  $\range(B_k^\star)$. These conditions constitute an
  \emph{identification boundary}: each is individually necessary,
  no two jointly sufficient
  ...
  Building on this identification result, we propose \emph{Single-Play
  Subspace-Calibrated Optimism} (SPSC), which interleaves probing with
  ...
  An $\Omega(\sqrt{cT})$ lower bound for explore-then-commit
  policies (Theorem~\ref{thm:etc_lower_bound}) confirms the
  probe--subspace tradeoff is fundamental.
  A stronger $\Omega(c^{1/3}T^{2/3})$ lower bound
  (Theorem~\ref{thm:probe_rate_lb}), under the condition that
  projected exploitation incurs $\Omega(\varepsilon)$ per-round
  regret from subspace error $\varepsilon$, matches the upper bound
  for projected-exploitation policies satisfying this condition
  (Remark~\ref{rem:eps_gap_resolved}).
  An adaptive variant detects change points without oracle boundaries
  and achieves the same leading rate with $O(K)$ overhead.

  \paragraph{Contributions.}
  Our work brings together low-rank bandits, piecewise-stationary LDS
  dynamics, and priced exploration in a single online decision-making
  framework.

  \begin{enumerate}[leftmargin=1.5em, itemsep=2pt]
  \item \textbf{Formulation.}
  We introduce an online decision-making model with
  piecewise-stationary low-rank rewards, stable within-segment LDS
  dynamics, and explicit single-play probing, evaluated through a
  \emph{costed dynamic regret} criterion (\S\ref{sec:formulation}).

  \item \textbf{Identification and impossibility.}
  Under three probe-side conditions---sub-Gaussian noise (unknown
  variance), bounded state--noise coupling, and full-dimensional
  coverage---we show that single-play probes yield quadratic
  measurements of the predictable second moment with controlled
  bias. Complementary impossibility results show each condition is
  individually necessary; approximate violations degrade gracefully
  (Appendix~\ref{app:omitted_identification}).

  \item \textbf{Algorithm.}
  We propose SPSC, which interleaves probing with optimistic
  exploitation \emph{directly} in the learned $r$-dimensional
  subspace via a windowed projected UCB rule. We give both an
  oracle-boundary version for analysis and an adaptive-reset version
  for unknown boundaries (\S\ref{sec:algorithm}).

  \item \textbf{Regret under exact and approximate probe conditions.}
  For a fixed number of stationary segments, we prove a dynamic
  regret bound of
  $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$,
  and show that the leading rate is preserved under
  $T^{-1/3}$-level probe-side misspecification.

  \item \textbf{Empirical validation.}
  Experiments on synthetic and real benchmarks show that SPSC
  consistently improves over strong baselines across a broad
  $(d,r)$ regime, with regret reductions of 6--38\%
...
Recent work achieves tighter rates via explore-then-refine~\citep{kang2022gestt}, two-to-infinity recovery~\citep{pmlr-v235-jedra24a}, optimal experimental design~\citep{pmlr-v235-jang24e}, and spectral methods~\citep{stojanovic2023spectral}. All assume a \emph{fixed} representation $B^\star$---none handle changing segment-wise representations that must be re-identified online.

\paragraph{Nonstationary and piecewise-stationary bandits.}
When the reward parameter $\theta_t$ varies over time, dynamic-regret analyses bound
$\sum_t (\max_x x^\top\theta_t - x_t^\top\theta_t)$ in terms of the variation budget $V_T := \sum_t \|\theta_{t+1} - \theta_t\|$ or the number of switches $K$.
Sliding-window LinUCB~\citep{russac2019weighted} and discounted methods~\citep{cheung2019learning} achieve $\widetilde O(d^{2/3}V_T^{1/3}T^{1/3})$; \citet{abbasi2023dynamic} give $\widetilde O(\sqrt{KdT})$ for piecewise-stationary $K$-armed bandits; and \citet{hou2024pslb} study best-arm identification under piecewise stationarity. Their regret scales with $d$, not $r$---because they do not recover or exploit a latent low-rank representation.

\paragraph{Active information acquisition and probing.}
In costly-observation bandits, the learner pays $c$ per observed reward:
\citet{seldin2014paid} study label-efficient prediction, \citet{tucker2023costly} prove $\Omega(c^{1/3}T^{2/3})$ lower bounds for $K$-armed bandits with costly observations, and \citet{elumar2025costly} analyze multi-armed bandits with costly probes.
Active query models~\citep{sekhari2023contextual} and system-identification methods~\citep{memmel2024asid} also study exploration under cost constraints.
...
The interaction between the probe cost $c|\cTprobe|$ and the subspace estimation rate $O(1/\sqrt{m})$ creates the $\widetilde O(T^{2/3})$ tradeoff term that is absent in stationary low-rank or unstructured nonstationary settings.
...
These conditions are probe-side identification requirements, not generic modeling restrictions; their practical scope and formal necessity are discussed in Appendix~\ref{sec:discussion}.
\begin{remark}[Online variance estimation at no asymptotic cost]
  \label{rem:burnin_free}
  Since $\sigma_\varepsilon^2$ is unknown, the algorithm estimates it from early probe-round residuals. Dedicating the first $T^{2/3}$ probe rounds to variance estimation yields $|\hat\sigma^2 - \sigma_\varepsilon^2| \le \delta_\sigma = O(T^{-1/3})$; this cost is absorbed into the existing $\widetilde{O}(T^{2/3})$ probe--subspace tradeoff term in Theorem~\ref{thm:closed_dynamic_regret_general}, so the estimation is \emph{free} in the regret bound. For isotropic Gaussian probes, the resulting bias $\widetilde{B} \approx \frac{\delta_\sigma}{2}I_d$ is a scaled identity that shifts eigenvalues uniformly without corrupting eigenvectors (Proposition~\ref{prop:biased_measurement}), so subspace recovery is unaffected.
  \end{remark}

The concentration analysis requires bounded noise; the standard truncation argument is in Appendix~\ref{app:concentration}(Remark~\ref{rem:truncation}).
...
\paragraph{Predictable second moment.} The central inferential target is the predictable second moment $\widetilde{M}_t := \E[\theta_t \theta_t^\top \mid \cH_{t-1}] = B_k^\star \E[w_t w_t^\top \mid \cH_{t-1}] (B_k^\star)^\top, \qquad t \in \cI_k.$ Its importance is that $\widetilde{M}_t$ encodes the active low-dimensional subspace through its range. At the segment level, define the probe-time average $\bar{M}_k^{\mathrm{probe}} := \frac{1}{m_k} \sum_{t \in \cT_k} \widetilde{M}_t.$ Here $\cT_k := \cTprobe \cap \cI_k$ denotes the probe rounds within segment $k$ and $m_k := |\cT_k|$ is their count. When the latent probe-time covariance has full rank $r$, this quantity identifies the active segment subspace.
...
\end{lemma}
Here $C_{\mathcal K}(\rho, L, d)$ denotes an explicit constant depending on the probe covariance lower bound $\rho$, the probe norm bound $L$, and the ambient dimension $d$; for isotropic Gaussian probes $u \sim \mathcal{N}(0, I_d)$ truncated to $\|u\| \le L$, $C_{\mathcal K} \le 1$ (see Remark~\ref{rem:probe_design} in the appendix). The proof is given in Appendix~\ref{app:all_proofs} (\S\ref{proof:K_invertible}).
%% ============================================================
%%  4. IDENTIFICATION
%% ============================================================
...
This section establishes the identification pipeline underlying the algorithm.
We first show that single-play probe rewards reveal quadratic information about the predictable second moment.
We then lift these quadratic observations into matrix-valued estimates whose leading eigenspace recovers the active subspace.
...
\paragraph{Lifted operator and subspace recovery.} The preceding results show that probe rounds identify the relevant quadratic form. To turn this into a usable matrix estimator, we invert the probe moment operator. Under Lemma~\ref{lem:K_invertible}, define the \emph{lifted probe sample} $G_t := \mathcal K^{-1}(s_t u_tu_t^\top).$ When $Q$ is the isotropic Gaussian $\mathcal{N}(0, I_d)$ (or its bounded truncation), the fourth-moment structure gives an explicit closed form: $\mathcal K^{-1}(N) = \tfrac{1}{2}N - \tfrac{\tr(N)}{2(d+2)}\,I_d$, so each lifted sample $G_t = \frac{1}{2}s_t u_t u_t^\top - \frac{s_t \|u_t\|^2}{2(d+2)}I_d$ is computable in $O(d^2)$ operations ($N \in \R_{\mathrm{sym}}^{d\times d}.$)
...
\begin{remark}[Identification boundary and practical scope]
  \label{rem:necessity_compact}
  Each probe-side condition is provably necessary (Appendix~\ref{app:non-identification}); all three are stated in their weakest usable form:
  \emph{(i)~Noise variance} is unknown and estimated online; burn-in cost $O(T^{2/3})$ is absorbed into the existing tradeoff term
  (Remark~\ref{rem:burnin_free}). Empirically, $5\times$ misspecification causes $<5\%$ regret change.
  \emph{(ii)~State--noise coupling} is bounded ($\epsilon_\times$), not required to be zero; the penalty $O(\epsilon_\times T/\lambda_{\min})$ is explicit in Theorem~\ref{thm:closed_dynamic_regret_general}. Even $\epsilon_\times{=}1$ changes regret by $<1\%$ empirically.
  \emph{(iii)~Probe realizability:} $\supp(Q) \subseteq \cA_t$ holds under $\epsilon$-greedy randomization. This is the sole non-relaxable condition: restricted coverage causes $\Omega(T)$ regret (Appendix~\ref{app:coverage_lower_bound}).
  \end{remark}
...
The data-driven extension is Algorithm~\ref{alg:adaptive_reset_control_main} in Appendix~\ref{app:projected_exploitation}.
\end{remark}


\paragraph{Computational cost.}
Each probe round requires forming the lifted sample $G_t = \mathcal{K}^{-1}(s_t u_t u_t^\top)$ in $O(d^2)$ time and updating the running sum $\sum G_t$ (also $O(d^2)$). After $m_k$ probes, the eigendecomposition of $\widehat{M}_k \in \R^{d \times d}$ costs $O(d^2 r)$ via a partial eigenvalue solver (e.g., Lanczos iteration), amortized over the segment. Each exploitation round involves projecting $|\cA_t|$ actions via $z_t(x) = \widehat{U}_{t-1}^\top x$ ($O(dr|\cA_t|)$), updating the $r \times r$ ridge system ($O(r^2)$), and solving the UCB maximization ($O(r^2 |\cA_t|)$). The total per-round cost is $O(dr|\cA_t| + r^2)$, which is linear in $d$ and quadratic in $r$---a significant reduction over the $O(d^2|\cA_t|)$ cost of ambient LinUCB when $r \ll d$.



\paragraph{Adaptive-reset variant.}
The known-boundary version isolates the identification-to-exploitation mechanism cleanly; the adaptive-reset version is the practically deployable form and inherits the same leading rate up to lower-order overhead.
Algorithm~\ref{alg:adaptive_reset_control_main} (Appendix~\ref{app:projected_exploitation}) extends Algorithm~\ref{alg:single_play_control_rdim} to the case of unknown segment boundaries through a data-driven change-point detector.
...
\end{equation}
A reset is triggered when $S_t$ exceeds a threshold $b$ calibrated to within-segment variation and concentration noise (the precise form is given in \S\ref{sec:theory}, Eq.~\eqref{eq:detector_threshold_main}).
After each detected reset, $m_{\mathrm{relearn}}$ consecutive probe rounds are used to rebuild the subspace estimate from scratch.
%% ============================================================
...
\label{sec:theory}
  ...
  This section presents the main regret guarantees. Theorem~\ref{thm:full_horizon_windowed} decomposes regret into probe cost,
  statistical regret, subspace mismatch, and drift (Appendix~\ref{app:all_proofs}); Theorem~\ref{thm:closed_dynamic_regret} optimizes
   probe allocation; Theorem~\ref{thm:closed_dynamic_regret_general} extends to weakened assumptions;
  Theorem~\ref{thm:adaptive_regret} handles unknown boundaries.
Proofs are in Appendix~\ref{app:all_proofs} (\S\ref{proof:full_horizon_windowed}, \S\ref{proof:closed_dynamic_regret},
  \S\ref{proof:regret_misspecified_variance}, and \S\ref{proof:adaptive_regret}).
  ...
  \label{eq:eps_k_rate}
  \end{equation}
  where $C_{\mathrm{sub}} = 8R_X/\lambda_{\min}$ is the explicit constant from Theorem~\ref{thm:projector_concentration_summary}
  (Appendix~\ref{app:concentration}), and $R_X := C_{\mathcal K}\,L^2\,R_s + S_w^2$, $R_s := (LS_w + L_\varepsilon)^2 +
  \sigma_\varepsilon^2$.

  The statistical confidence radius is
  \begin{equation}
  \beta_t^{(r,W)} := \sigma_\varepsilon
  \sqrt{r \log\!\left(1+\frac{W R_{\cA}^2}{\lambda r}\right) + 2\log(1/\delta)} + \sqrt{\lambda}S_w.
  \label{eq:beta_rW}
  \end{equation}
  The correction coefficient is $\gamma_t := \Gamma_k + V_{k,t}(W)$, where $\Gamma_k := S_w \varepsilon_k$ is the subspace-mismatch
  level and $V_{k,t}(W) := \sum_{s\in E_k,\, t-W \le s < t}\|\theta_{s+1}-\theta_s\|_2$ is the local variation.
  ...
  with
    probability at least $1-\delta$, $\DynReg_T^{(c)}$ decomposes into probe cost ($\propto \sum_k m_k$), projected statistical
  regret
    ($\propto \sum_k \beta_k^{(r,W)}\sqrt{rn_k}$), subspace mismatch ($\propto \sum_k \varepsilon_k n_k$), and drift ($\propto
  W\sum_k
    V_k$). The full bound is in Appendix~\ref{app:all_proofs}, \S\ref{proof:full_horizon_windowed}.
    \end{theorem}
  ...
  $A := 2R_{\cA} S_w + c$ and $B := 2 C_{\mathrm{sub}} S_w R_{\cA} \sqrt{\log(d/\delta)}$.
  For a fixed window length $W\ge 1$, let $L_W := \log(1+WR_{\cA}^2/(\lambda r))$ and
  ...
  The three terms have clear roles: $\widetilde O(r\sqrt{T})$ is the projected exploitation cost (vs.\ $\widetilde O(d\sqrt{T})$ in
  ambient space), $\widetilde O(T^{2/3})$ is the probe--subspace tradeoff, and $O(V)$ is the irreducible drift price.
  ...
  Suppose Assumptions~\ref{assum:latent_dynamics}--\ref{assum:probe_noise} hold and every true change satisfies $\Delta_k \ge
...
\end{equation}
  ...
  For fixed $K$, the adaptive algorithm achieves the same leading rate $\widetilde O(r\sqrt{T}) + \widetilde O(T^{2/3}) + O(V)$ with
  $O(K)$ lower-order overhead.
  \end{theorem}
...
%% ============================================================
 ...
 \section{Experiments}
  ...
  We evaluate SPSC on synthetic and real-data benchmarks.
  Unless otherwise stated, all curves report the mean over multiple random seeds with $\pm 1$ standard-error bands.
  The main paper presents four experiments; Appendix~\ref{app:additional_experiments} contains 14 additional studies
  (Table~\ref{tab:experiment_roadmap}) including sensitivity analyses and the Warfarin clinical dosing benchmark
  (Appendix~\ref{app:warfarin}).
  Setup details are in Appendix~\ref{app:synthetic_setup}.

  \paragraph{Experiment 1: Phase transition (Theorems~\ref{thm:closed_dynamic_regret},~\ref{thm:ambient_regret}).}
  Figure~\ref{fig:synthetic-phase} shows the SPSC/LinUCB regret ratio over a broad $(d,r)$ grid. The empirical crossover boundary
  closely tracks $d{-}r = T^{1/6}$: LinUCB wins at $d \le 20$, SPSC dominates at $d \ge 45$ with $2$--$34\%$ reductions, and every
  $r$-curve tracks $\sqrt{r/d}$, validating the leading-rate improvement from $d\sqrt{T}$ to $r\sqrt{T}$.

    \begin{figure}[!htpb]
        \centering
        \includegraphics[width=0.84\textwidth]{experiment1_synthetic_phase.png}
        \caption{Synthetic phase-transition benchmark.
        \textbf{(a)}~SPSC/LinUCB ratio contour.
        Solid black: empirical ratio${=}1$.
        Dashed grey: $d{-}r = T^{1/6}$.
        \textbf{(b)}~Ratio vs.\ $d$ (log-scale);
        dotted: $\sqrt{r/d}$ theory.}
  ...
  best non-oracle at $d \ge 55$, $r \ge 10$, beating Restart-LinUCB by $27\%$, SW-LinUCB by $29\%$, D-LinUCB by $30\%$ at $d{=}155$.
  At small $d$, probe cost dominates, confirming the theory predicts \emph{where} SPSC helps, not just \emph{that} it helps.
  ...
  \textbf{0.76} \\
    ...
    \paragraph{Experiment 3: Approximate low rank (Theorem~\ref{thm:closed_dynamic_regret_general}).}
    On warfarin dose selection (Appendix~\ref{app:warfarin}), SPSC reduces regret by
    $26$ to $32\%$ vs.\ all nonstationary baselines at every tested rank. The data is only \emph{approximately} low rank
  (subspace error
    $\approx 0.96$ to $1.00$), validating Theorem~\ref{thm:closed_dynamic_regret_general}: the penalty from approximate
  structure is
    absorbed into the $O((\delta_\sigma + \epsilon_\times)T/\lambda_{\min})$ term without destroying the leading rate. Probe
   overhead
    is $3.3$ to $3.6\%$.
    A random subspace ablation (Appendix~\ref{app:warfarin_ablation}) decomposes the source of the gain: roughly half comes
  from
    dimensionality reduction ($\sqrt{r}$ vs.\ $\sqrt{d}$ confidence radius) and the other half from the learned subspace,
  which
    retains $2$ to $6\times$ more signal variance than random projection.

  \paragraph{Experiment 4: Mechanism validation.}
  Three experiments validate the identification boundary (Appendix~\ref{app:robustness_necessity}): $5\times$ variance
  misspecification changes regret by $<5\%$; cross-correlation $\epsilon_\times{=}1$ changes regret by $<1\%$; restricting coverage
  causes $\Omega(T)$ blow-up. These confirm not just performance but the \emph{mechanism}: the first two conditions degrade
  gracefully (Theorem~\ref{thm:closed_dynamic_regret_general}), while the third is provably non-relaxable
  (Theorem~\ref{thm:restricted_span_minimax_regret}). The probe-rate ablation (Appendix~\ref{app:probe_ablation}) confirms the
  $T^{2/3}$ U-shaped tradeoff.
...
We characterize the \emph{probe side identification boundary} for recovering a changing low rank subspace from single play
   scalar feedback: under standing latent excitation and stability conditions, three conditions (unknown variance estimated
  online, bounded state noise coupling, full dimensional coverage) are each necessary, and their joint satisfaction enables
  quadratic measurements of the predictable second moment with controlled bias.
Building on this, SPSC achieves $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ costed dynamic regret; an adaptive variant removes oracle segment boundaries.
Experiments confirm regret reductions over all nonstationary baselines on real data when $d/r$ is large.




\paragraph{Broader impact and future work.}
This work is primarily theoretical, applicable to recommendation, pricing, and control. The $\Omega(c^{1/3}T^{2/3})$ lower bound (Theorem~\ref{thm:probe_rate_lb}) matches the upper bound for projected-exploitation policies under a natural per-round regret condition; the remaining open question is whether non-projecting algorithms can achieve $O(\sqrt{cT})$. Other directions include unknown rank $r$ and time-varying transition matrices.
...
\appendix
  ...
  \tocentry{E}{app:supporting_statements}{Supporting Statements}
    ...
    \tocsubentry{E.11}{app:matching_lower_bound}{Matching $\Omega(c^{1/3}T^{2/3})$ Lower Bound}
...
\label{sec:discussion}
 ...
 Concretely, we outline three deployable instantiations:
  ...
  dimensions during those rounds.
    ...
    \paragraph{When full coverage does not hold.}
    The requirement fails when the learner cannot randomize freely, for example in safety constrained systems where certain
  action
    dimensions are locked, or on platforms where the item pool does not span $\R^d$.
    In such settings our guarantees do not apply: Theorem~\ref{thm:restricted_span_minimax_regret} shows that restricting
    probes to a proper subspace $S \subsetneq \R^d$ causes $\Omega(T)$ regret, and the Experiment~C ablation
    ...
    guarantees is an open question; the lifted operator approach requires invertibility of $\mathcal{K}$, which needs both full-rank $\E[uu^\top]$ and a non-degenerate fourth-moment tensor (Lemma~\ref{lem:K_invertible}).
Theorems~\ref{thm:restricted_span_minimax_regret}--\ref{thm:two_point_testing_incomplete_coverage}
  and Experiment~C show that relaxing full coverage causes
  ...
  Theorem~\ref{thm:closed_dynamic_regret_general} and Experiment~A show
  ...
  Corollary~\ref{cor:regret_cross_correlation} and Experiment~B show
...
\paragraph{Probe-cost lower bounds.}
  ...
  Building on this, Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes a matching $\Omega(c^{1/3}T^{2/3})$ lower bound for projected-exploitation policies under the condition that subspace error $\varepsilon$ incurs $\Omega(\varepsilon)$ per-round regret.
  Hard projection onto a misaligned subspace provides the mechanism for this linear-in-$\varepsilon$ loss; in particular, SPSC's hard-projection exploitation satisfies this condition.
  Theorem~\ref{thm:probe_rate_lb} thus shows that the $\widetilde{O}(c^{1/3}T^{2/3})$ probe--subspace tradeoff term in Theorem~\ref{thm:closed_dynamic_regret} is tight for this policy class.

  The only remaining open question is whether an algorithm operating in the full $d$-dimensional ambient space (without projecting onto the estimated subspace) could achieve $O(\varepsilon^2)$ per-round overhead and thus $O(\sqrt{cT})$ total regret.
...
\midrule
 SPSC (ours) & This work & $\widetilde O(r\sqrt{T} + c^{1/3}T^{2/3})$ & Non-stationary; learns $B$ via probing \\
\bottomrule
\end{tabular}
...
Thm.~\ref{thm:etc_lower_bound} \newline (ETC lower bound) &
  ...
  Confirms probe--subspace tradeoff is fundamental \\
...
Thm.~\ref{thm:probe_rate_lb} \newline (Matching $\Omega(c^{1/3}T^{2/3})$) &
  Conditional matching lower bound for projected-exploitation policies under linear-in-$\varepsilon$ loss &
  Matches upper bound for this class (Remark~\ref{rem:eps_gap_resolved}) \\
...
\subsection{\texorpdfstring{Matching $\Omega(c^{1/3}T^{2/3})$ Lower Bound for Projected Exploitation}{Matching Lower Bound for Projected Exploitation}}
...
Under Assumptions~\ref{assum:latent_dynamics}--\ref{assum:probe_noise} with $d \ge 2r$, suppose that any policy committing to a subspace estimate $\widehat{P}$ with $\|\widehat{P} - P^\star\|_{\op} = \varepsilon$ and exploiting in the estimated subspace incurs per-round expected regret at least $\mu\varepsilon$ for a constant $\mu > 0$.  Then there exists $c_4 > 0$ such that for all $T$ sufficiently large,
\[
...
The following lemma shows that the linear-in-$\varepsilon$ loss condition of Theorem~\ref{thm:probe_rate_lb} is genuinely realized by hard-projection scoring on a concrete bounded-action instance.

\begin{lemma}[Linear loss for hard-projection scoring]
\label{lem:hard_projection_linear_loss}
Let $d=2$, $r=1$, and let the true projector be $P^\star = e_1 e_1^\top$.
Let the estimated projector be $\widehat{P} = uu^\top$ with $u = (\cos\alpha, \sin\alpha)$ and $\sin\alpha = \varepsilon \in (0, 1/4]$.
Let $\theta = S_w e_1$ and define the action set $\cA_\varepsilon := \{0,\, x_\varepsilon\}$ where
\[
x_\varepsilon := \frac{R_{\cA}}{\sqrt{1+a^2}}(a,\,-1), \qquad a := \tfrac{1}{2}\tan\alpha.
\]
Consider the hard-projection scoring rule $x \mapsto x^\top \widehat{P}\theta$.
Then for all sufficiently small $\varepsilon$: (i) the true best action is $x_\varepsilon$; (ii) the scoring rule selects $0$; and (iii) the per-round regret satisfies
\[
\max_{x \in \cA_\varepsilon} x^\top\theta - x_{\mathrm{HP}}^\top\theta \ge c_{\mathrm{hp}}\, R_{\cA}\, S_w\, \varepsilon
\]
for an absolute constant $c_{\mathrm{hp}} > 0$.
\end{lemma}

\begin{proof}
\emph{True reward.}
Since $\theta = S_w e_1$, the reward of the nonzero arm is
$x_\varepsilon^\top\theta = R_{\cA} S_w\, a / \sqrt{1+a^2}$.
With $a = \frac{1}{2}\tan\alpha$ and $\sin\alpha = \varepsilon$, we have $a = \varepsilon/(2\cos\alpha) \asymp \varepsilon$ for small $\varepsilon$, so $x_\varepsilon^\top\theta \asymp R_{\cA} S_w \varepsilon > 0$.
The true best action is $x_\varepsilon$.

\emph{Hard-projection score.}
Since $\widehat{P} = uu^\top$ and $u^\top\theta = S_w\cos\alpha$:
\[
x_\varepsilon^\top \widehat{P}\theta
= (x_\varepsilon^\top u)(u^\top\theta)
= \frac{R_{\cA} S_w}{\sqrt{1+a^2}}(a\cos\alpha - \sin\alpha)\cos\alpha.
\]
Substituting $a = \frac{1}{2}\tan\alpha$:
\[
a\cos\alpha - \sin\alpha = \tfrac{1}{2}\sin\alpha - \sin\alpha = -\tfrac{1}{2}\sin\alpha,
\]
so $x_\varepsilon^\top \widehat{P}\theta = -\frac{1}{2}\frac{R_{\cA} S_w}{\sqrt{1+a^2}}\sin\alpha\cos\alpha < 0$.
Since $0^\top \widehat{P}\theta = 0$, the scoring rule selects $0$.

\emph{Regret.}
The chosen action has true reward $0$; the optimal action has reward $x_\varepsilon^\top\theta = R_{\cA} S_w\, a/\sqrt{1+a^2} \ge c_{\mathrm{hp}} R_{\cA} S_w \varepsilon$ for a constant $c_{\mathrm{hp}} > 0$ and all sufficiently small $\varepsilon$.
\end{proof}

\begin{remark}[Scope of Lemma~\ref{lem:hard_projection_linear_loss}]
\label{rem:hard_projection_scope}
Lemma~\ref{lem:hard_projection_linear_loss} shows that the linear-in-$\varepsilon$ loss condition in Theorem~\ref{thm:probe_rate_lb} is genuinely realized by hard-projection scoring on a bounded-action instance.
It does not by itself prove the minimax lower bound of Theorem~\ref{thm:probe_rate_lb}, because the latter additionally requires coupling this linear-loss mechanism to the subspace-recovery lower bound of Theorem~\ref{thm:minimax_subspace_recovery} over a common lower-bound family.
\end{remark}

\begin{remark}[The $\varepsilon$ vs.\ $\varepsilon^2$ resolution]
\label{rem:eps_gap_resolved}
Theorem~\ref{thm:sqrt_ct_all} uses the information-theoretic $\Omega(\varepsilon^2)$ per-round cost, yielding $\Omega(\sqrt{cT})$.
Theorem~\ref{thm:probe_rate_lb} uses the $\Omega(\varepsilon)$ per-round cost inherent to projected exploitation, yielding $\Omega(c^{1/3}T^{2/3})$, which matches the upper bound in Theorem~\ref{thm:closed_dynamic_regret}.
Lemma~\ref{lem:hard_projection_linear_loss} provides a concrete construction showing that hard-projection scoring realizes this linear-in-$\varepsilon$ loss on a bounded-action instance.
Whether an algorithm operating in the full ambient space can achieve $O(\varepsilon^2)$ per-round overhead remains open.
\end{remark}

\begin{remark}[Matching upper bound]
SPSC achieves $\widetilde{O}(\sqrt{rT}) + \widetilde{O}(c^{1/3}T^{2/3}) + O(V)$ via projected exploitation with $m \asymp (T/c)^{2/3}$ probes per segment.  Theorem~\ref{thm:probe_rate_lb} shows that the $c^{1/3}T^{2/3}$ term is tight for projected-exploitation policies satisfying the linear-in-$\varepsilon$ loss condition.
\end{remark}
...
\begin{proof}
The proof follows the same probe-cost tradeoff structure as Theorem~\ref{thm:sqrt_ct_all}, but with the stronger per-round regret scaling that holds for projected-exploitation policies.

\textbf{Per-round cost of subspace error.}
By hypothesis, when the subspace projector estimate satisfies $\|\widehat{P} - P^\star\|_{\op} = \varepsilon$, each exploitation round incurs expected regret at least $\mu\varepsilon$.  This linear-in-$\varepsilon$ scaling arises because exploitation in the learned subspace requires projecting actions onto $\range(\widehat{P})$, and the projection introduces an $O(\varepsilon)$ per-round bias.  Specifically, for any action $x$ with $\|x\| \le L_x$ and any $\theta_t$ with $\|\theta_t\| \le S_w$:
\[
|x^\top(\widehat{P} - P^\star)\theta_t| \le \|x\| \cdot \|\widehat{P} - P^\star\|_{\op} \cdot \|\theta_t\| \le L_x S_w \varepsilon.
\]
This bias cannot be eliminated by any exploitation strategy that uses the estimated subspace for dimensionality reduction, because the policy has no access to the signal component in directions $\range(P^\star) \setminus \range(\widehat{P})$.

\textbf{Substitution of minimax rate.}
By Theorem~\ref{thm:minimax_subspace_recovery}, after $m$ probes, $\sup_\nu \E_\nu[\|\widehat{P} - P^\star\|_{\op}] \ge c_1/\sqrt{m}$.  The exploitation regret over $T - m$ rounds is therefore at least
\[
...
\]
Substituting $m^\star$ back into the lower bound:
\begin{align}
cm^\star + \frac{\gamma T}{2\sqrt{m^\star}}
&= c\left(\frac{\gamma T}{4c}\right)^{2/3} + \frac{\gamma T}{2}\left(\frac{4c}{\gamma T}\right)^{1/3} \notag \\
&= \frac{(\gamma T)^{2/3}}{(4)^{2/3}\, c^{-1/3}} \cdot c + \frac{\gamma T}{2} \cdot \frac{(4c)^{1/3}}{(\gamma T)^{1/3}} \notag \\
&= \frac{(\gamma)^{2/3}}{4^{2/3}} c^{1/3} T^{2/3} + \frac{(4)^{1/3}}{2} (\gamma)^{2/3} c^{1/3} T^{2/3} \notag \\
&= \frac{3}{2}\left(\frac{\gamma}{4}\right)^{2/3} c^{1/3} T^{2/3}. \label{eq:probe_rate_lb_final}
\end{align}
It remains to verify $m^\star \le T/2$: since $m^\star = (\gamma T/(4c))^{2/3}$ and $\gamma, c$ are constants, $m^\star = o(T)$ for $T$ sufficiently large.

Setting $c_4 := \frac{3}{2}(\gamma/4)^{2/3} = \frac{3}{2}(\mu c_1/4)^{2/3}$ completes the proof.
\end{proof}
...
Table~\ref{tab:experiment_roadmap} provides a complete roadmap: each row links one experiment to the theoretical prediction it
 ...
 {%
  ...
  $d{\in}\{10,...,80\}$, $r{\in}\{1,...,20\}$, $K{=}10$, $T{=}5000$; 3 seeds &
...
(Figure~\ref{fig:synthetic-phase}).
 ...
 \paragraph{Design rationale.}
  ...
  well-conditioned at all $d$ and SPSC cannot overcome the probe
  overhead---a setting that does not reflect practical feature
  distributions.
...
30 & 15 & 1209 & 1283 &  723 & \textbf{0.94} & 0.707 \\
30 & 20 & 1193 & 1257 &  895 & 0.95 & 0.816 \\
\midrule
45 &  1 & 2235 & 2282 &    9 & 0.98 & 0.149 \\
45 &  3 & 1845 & 2192 &  113 & \textbf{0.84} & 0.258 \\
45 &  5 & 1668 & 1999 &  218 & \textbf{0.83} & 0.333 \\
45 & 10 & 1270 & 1611 &  457 & \textbf{0.79} & 0.471 \\
45 & 15 & 1251 & 1456 &  692 & \textbf{0.86} & 0.577 \\
45 & 20 & 1335 & 1556 &  919 & \textbf{0.86} & 0.667 \\
\midrule
60 &  1 & 2306 & 2694 &    8 & \textbf{0.86} & 0.129 \\
60 &  3 & 1752 & 2532 &  108 & \textbf{0.69} & 0.224 \\
60 &  5 & 1674 & 2433 &  235 & \textbf{0.69} & 0.289 \\
60 & 10 & 1335 & 1900 &  497 & \textbf{0.70} & 0.408 \\
60 & 15 & 1272 & 1640 &  681 & \textbf{0.78} & 0.500 \\
60 & 20 & 1232 & 1484 &  838 & \textbf{0.83} & 0.577 \\
\midrule
80 &  1 & 2391 & 2878 &    6 & \textbf{0.83} & 0.112 \\
80 &  3 & 2050 & 2978 &  120 & \textbf{0.69} & 0.194 \\
80 &  5 & 1674 & 2544 &  222 & \textbf{0.66} & 0.250 \\
80 & 10 & 1368 & 1881 &  449 & \textbf{0.73} & 0.354 \\
80 & 15 & 1249 & 1644 &  631 & \textbf{0.76} & 0.433 \\
80 & 20 & 1273 & 1614 &  845 & \textbf{0.79} & 0.500 \\
\bottomrule
\end{tabular}
\end{table}
