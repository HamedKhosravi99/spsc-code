...
\begin{abstract}
  Low-rank structure enables sample-efficient sequential decision-making when the reward parameter
  $\theta_t \in \R^d$ lies in an $r$-dimensional subspace ($r \ll d$), but existing methods either
  require a fixed, known representation or pay the full ambient cost $\widetilde{O}(d\sqrt{T})$.
  When the subspace changes across $K$ segments and the learner receives only a single scalar
  $y_t = x_t^\top\theta_t + \varepsilon_t$ per round, recovering the active subspace from such
  projections is a non-trivial identification problem, and each probe round used for subspace
  sensing incurs an explicit cost $c > 0$.
  We characterize a sharp identification boundary for this problem: three probe-side conditions
  (sub-Gaussian noise with online variance estimation, bounded state--noise coupling, and
  full-dimensional probe coverage) are each individually necessary---with formal impossibility
  results for every violation (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c})---and jointly
  sufficient for subspace recovery via a lifted matrix operator.
  Building on this, we propose \emph{Single-Play Subspace-Calibrated Optimism} (SPSC), which
  achieves $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ costed dynamic regret---replacing
  the ambient $d\sqrt{T}$ dependence with intrinsic rank $r$---and prove a matching
  $\Omega(c^{1/3}T^{2/3})$ lower bound on the probe--subspace tradeoff term within the
  projected-exploitation policy class, the class containing all known algorithms for this problem.
  Experiments on synthetic and real-data benchmarks including clinical dosing are consistent with
  the predicted phase-transition boundary $d - r \asymp T^{1/6}$ and show 6--38\% regret
  reductions over ambient-space baselines.
  \end{abstract}
%% ============================================================
%%  1. INTRODUCTION
%% ============================================================
...
\label{sec:theory}
  ...
  In practice, the learner applies thresholding to the \emph{sample} eigenvalues of $\widehat{M}_k$
  using the computable threshold $2\tau_k$; Weyl's inequality guarantees that when the population gap
  exceeds $4\tau_k$, the sample gap exceeds $2\tau_k$ with the stated probability.
