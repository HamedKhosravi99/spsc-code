...
\begin{abstract}
  ...
  regret $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$.
    For the class of hard-projection score-maximizing policies---which includes SPSC---a matching unconditional $\Omega(c^{1/3}T^{2/3})$ lower bound confirms the probe--subspace tradeoff is tight for this class; an
...
\paragraph{Probe-cost lower bounds.}
  ...
  Building on this, Theorem~\ref{thm:probe_rate_lb} (Appendix~\ref{app:matching_lower_bound}) establishes an unconditional matching $\Omega(c^{1/3}T^{2/3})$ lower bound for hard-projection score-maximizing policies (Definition~\ref{def:hard_projection_policy_class})---a class that includes SPSC---via a fixed two-point lower-bound family satisfying Assumption~\ref{assum:probe_realizability}.
