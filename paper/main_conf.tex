...
not~$r$.
  ...
  \item \textbf{Identification boundary.}
  Necessary-and-sufficient probe-side conditions for subspace recovery from single-play scalar feedback: three conditions each individually necessary, jointly sufficient via a lifted matrix operator
  (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}, \ref{prop:quad}--\ref{prop:lifted_probe_unbiased}).

  \item \textbf{Intrinsic-rank upper bound.}
  SPSC achieves $\widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$ costed dynamic regret, with an adaptive variant removing oracle boundaries
  (Theorems~\ref{thm:closed_dynamic_regret},~\ref{thm:adaptive_regret}).

  \item \textbf{Matching lower bound.}
  $\Omega(c^{1/3}T^{2/3})$ within the projected-exploitation class, showing the tradeoff is sharp
  (Theorem~\ref{thm:probe_rate_lb}).

  \item \textbf{Robustness.}
  Leading rate preserved under $T^{-1/3}$-level probe misspecification (Theorem~\ref{thm:closed_dynamic_regret_general}); crossover $d - r > C\,T^{1/6}$ gives a finite-$T$ rule-of-thumb (\S\ref{sec:experiments}).

  \item \textbf{Theorem-relevant experiments.}
  Phase-transition boundary, $T^{2/3}$ tradeoff, and identification conditions tested on synthetic and clinical benchmarks, with 6--38\% regret reductions (\S\ref{sec:experiments}).
  \end{enumerate}
...
\label{sec:theory}
  ...
  Theorem~\ref{thm:etc_lower_bound} (Appendix~\ref{app:probe_cost_lower_bound}) proves $\Omega(\sqrt{cT})$ for all explore-then-commit policies.
  ...
  The gap between $\Omega(\sqrt{cT})$ (unconditional) and $\Omega(c^{1/3}T^{2/3})$ (projected-exploitation) opens a precise question (Remark~\ref{rem:eps_gap_resolved}): can non-projecting algorithms achieve $O(\sqrt{cT})$?
...
Limitations and open problems---including closing the gap between projecting and non-projecting lower bounds, unknown rank with gradual spectral decay, and time-varying dynamics---are discussed in Appendix~\ref{sec:discussion}.
