...
$K$ segments.
Existing low-rank bandit methods assume a fixed representation;
nonstationary methods adapt to changing parameters but pay the full ambient cost
$\widetilde{O}(d\sqrt{T})$.
No prior work establishes when a changing low-rank subspace is even
\emph{identifiable} from single-play scalar feedback, let alone how to exploit
such structure optimally.
...
for every violation) and jointly sufficient for subspace recovery via a lifted matrix
operator $G_t = \mathcal{K}^{-1}(s_t\,u_tu_t^\top)$.
This identification result is the first of its kind for changing low-rank structure under
scalar observations; all subsequent results---regret bounds and lower bounds
alike---follow from this characterization.

We establish three matched lower bounds that together determine the fundamental
...
(ii)~within the projected-exploitation class $\Pi_{\mathrm{HP}}$---containing all
known algorithms for this problem---a matching $\Omega(c^{1/3}T^{2/3})$ lower bound
...
As an algorithmic consequence, we prove that interleaving projected UCB exploitation
with probing achieves costed dynamic regret
$\DynReg_T^{(c)} = \widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$,
replacing the ambient $d\sqrt{T}$ dependence with intrinsic rank~$r$,
and an adaptive variant removes oracle segment boundaries at the same leading rate.
\end{abstract}
...
Two lower bounds clarify the cost of projection-based control under subspace
uncertainty: $\Omega(\sqrt{cT})$ for all admissible policies
(Theorem~\ref{thm:sqrt_ct_all}), and a matching $\Omega(c^{1/3}T^{2/3})$ within
the projected-exploitation class $\Pi_{\mathrm{HP}}$---containing all
known algorithms for this problem
(Theorem~\ref{thm:probe_rate_lb}).
The gap between these bounds isolates a precise open question:
whether non-projecting algorithms can achieve $O(\sqrt{cT})$ total regret.
...
Theorem~\ref{thm:probe_rate_lb} shows that the $c^{1/3}T^{2/3}$ term is tight for
the hard-projection score-maximizing class.
\end{remark}

\begin{remark}[Scope of the sharp lower bound]
\label{rem:sharp_lb_scope}
Theorem~\ref{thm:probe_rate_lb} fully closes the probe-cost lower bound for
the hard-projection score-maximizing class of Definition~\ref{def:hard_projection}.
It does not claim the same unconditional $\Omega(c^{1/3}T^{2/3})$ lower bound for
every possible projected-exploitation policy, nor for algorithms that operate
directly in the ambient space without committing to a projected score representation.
\end{remark}
...
\citet{mueller2019dynamic} combine low-rank structure with nonstationarity
for dynamic pricing at Amazon, but in a bandit convex optimization framing
