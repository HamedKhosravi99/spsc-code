...
\title{Identification Boundaries for Piecewise Stationary Low-Rank Bandits:\\
When Is a Changing Subspace Recoverable from Scalar Feedback?}
...
%% ============================================================
\begin{abstract}
Consider a learner who, at each round $t=1,\ldots,T$, selects an action
$x_t$ from a set $\cA_t\subset\R^d$ and observes a scalar reward
$y_t = x_t^\top\theta_t + \varepsilon_t$, where $\theta_t = B_k^\star w_t$
lies in a changing $r$-dimensional subspace ($r \ll d$) that is piecewise constant over
$K$ segments.
Existing low-rank bandit methods~\citep{jun2019bilinear,pmlr-v235-jedra24a,lu2021lowrank}
exploit rank structure to achieve $\widetilde{O}(r\sqrt{T})$ regret, but require a
\emph{fixed} representation matrix $B^\star$; nonstationary
methods~\citep{russac2019weighted,cheung2019learning} adapt to changing parameters but
pay the full ambient cost $\widetilde{O}(d\sqrt{T})$.
No prior work simultaneously achieves regret scaling with intrinsic rank~$r$,
adaptation to a changing subspace, and explicit accounting for the cost of acquiring
subspace information.

We close this gap.
Our main contribution is a sharp probe-side identification boundary:
three structural conditions---sub-Gaussian noise with online variance estimation,
bounded state--noise coupling $\|\E[\varepsilon_t\theta_t\mid\cH_{t-1},u_t]\|_2\le\epsilon_\times$,
and full-dimensional probe coverage $\E[uu^\top]\succeq\rho I_d$---are each individually
necessary (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c}, with formal impossibility
for every violation) and jointly sufficient for subspace recovery from single-play scalar
feedback via a lifted matrix operator $G_t = \mathcal{K}^{-1}(s_t\,u_tu_t^\top)$.
This identification result is the first of its kind for changing low-rank structure under
scalar observations.

Building on this characterization, we propose Single-Play Subspace-Calibrated Optimism
(SPSC) and prove a costed dynamic regret bound
$\DynReg_T^{(c)} = \widetilde{O}(r\sqrt{T}) + \widetilde{O}(T^{2/3}) + O(V)$,
replacing the ambient $d\sqrt{T}$ dependence with intrinsic rank~$r$.
We establish three matched lower bounds:
(i)~the $1/\sqrt{m}$ subspace recovery rate is minimax optimal via Fano's inequality
over Grassmannian packings (Theorem~\ref{thm:minimax_subspace_recovery});
(ii)~within the projected-exploitation class $\Pi_{\mathrm{HP}}$---containing SPSC and
all known algorithms for this problem---a matching $\Omega(c^{1/3}T^{2/3})$ lower bound
shows the probe--subspace tradeoff is sharp (Theorem~\ref{thm:probe_rate_lb});
(iii)~an unconditional $\Omega(\sqrt{cT})$ lower bound holds for all admissible policies
(Theorem~\ref{thm:sqrt_ct_all}).
Full-dimensional probe coverage is information-theoretically necessary:
restricting probes to any proper subspace forces $\Omega(T)$ regret
...
In the stationary setting $\theta_t=\theta$, recent
work~\citep{kang2022gestt,pmlr-v235-jedra24a,pmlr-v235-jang24e,lu2021lowrank}
achieves $\widetilde{O}(r\sqrt{T})$ regret via explore-then-refine, spectral methods,
and optimal design.
\citet{pmlr-v235-jedra24a} prove
$\widetilde{O}(r^{5/4}(m{+}n)^{3/4}\sqrt{T})$;
\citet{kang2022gestt} achieve
$\widetilde{O}(\sqrt{(d_1{+}d_2)MrT})$ for bilinear bandits.
All assume a \emph{fixed} representation matrix $B^\star$---none handle piecewise-changing
subspaces that must be re-identified online from scalar feedback.

\emph{Nonstationary and piecewise-stationary bandits.}
Dynamic-regret analyses bound regret in terms of variation $V_T$ or switch count~$K$.
Sliding-window methods~\citep{russac2019weighted,cheung2019learning} achieve
$\widetilde{O}(d^{2/3}V_T^{1/3}T^{1/3})$;
\citet{abbasi2023dynamic} give $\widetilde{O}(\sqrt{KdT})$ for $K$-armed nonstationary
bandits.
All scale with the ambient dimension~$d$, not the intrinsic rank~$r$---none exploit latent
low-rank structure.
...
\paragraph{What we do and how we push the state of the art.}
...
\item \textbf{Identification boundary (foundational contribution).}
  We prove that subspace identifiability from single-play scalar feedback is governed
  ...
  Each condition is \emph{individually necessary}---removing any one destroys
  identifiability entirely
  (Propositions~\ref{prop:nonid_a}--\ref{prop:nonid_c})---and all three are
  \emph{jointly sufficient} for subspace recovery via the centered probe statistic
  ...
  under scalar observations, and it is the result from which all other contributions
  in this paper follow.

\item \textbf{Minimax-optimal subspace recovery.}
  We prove that the $1/\sqrt{m}$ projector concentration rate achieved by our
  Davis--Kahan bound (Theorem~\ref{thm:projector_concentration_summary}) is minimax
  optimal: via Fano's inequality over Grassmannian packings of dimension
  $r(d{-}r)$, any estimator satisfies
  $\inf_{\widehat{P}}\sup_\nu\E[\|\widehat{P}-P^\star\|_{\op}] \ge c_1/\sqrt{m}$
  (Theorem~\ref{thm:minimax_subspace_recovery}).

\item \textbf{Regret-optimal algorithm.}
  As an algorithmic consequence of the identification boundary, SPSC interleaves
  probing with projected UCB exploitation in the learned $r$-dimensional subspace.
  For $K$ stationary segments with total within-segment drift
  $V := \sum_k\sum_{t\in\cI_k}\|\theta_{t+1}-\theta_t\|_2$, the costed dynamic regret
  satisfies (Theorem~\ref{thm:closed_dynamic_regret})
  \[
    \DynReg_T^{(c)} = \widetilde{O}(r\sqrt{T})
    + \widetilde{O}(T^{2/3}) + O(V),
  \]
  replacing the ambient $d\sqrt{T}$ dependence with intrinsic rank~$r$.
  An adaptive variant removes oracle boundaries at the same leading rate
  (Theorem~\ref{thm:adaptive_regret}).

\item \textbf{Matched lower bounds.}
  Two lower bounds clarify the cost of projection-based control under subspace uncertainty:
  \begin{itemize}[nosep]
    \item $\Omega(\sqrt{cT})$ for all admissible policies
    (Theorem~\ref{thm:sqrt_ct_all}), establishing an unconditional
    information-theoretic barrier;
    \item within the projected-exploitation class $\Pi_{\mathrm{HP}}$---containing SPSC
    and all known algorithms for this problem---a matching
    $\Omega(c^{1/3}T^{2/3})$ lower bound
    (Theorem~\ref{thm:probe_rate_lb}), showing the $T^{2/3}$ tradeoff is sharp.
  \end{itemize}
  The gap between these two bounds isolates a precise open question: whether
  non-projecting algorithms can achieve $O(\sqrt{cT})$ total regret.

\item \textbf{Information-theoretic necessity of full coverage.}
  Full-dimensional probe coverage is not an artifact of our analysis: restricting
  actions to any proper subspace $S\subsetneq\R^d$ forces $\Omega(T)$ minimax
  regret (Theorem~\ref{thm:restricted_span_minimax_regret}), even when the learner
  is allowed to probe within~$S$
...
\item \textbf{Robustness and regime prediction.}
  ...
  (Theorem~\ref{thm:closed_dynamic_regret_general}); the crossover
  $d-r > C\,T^{1/6}$ gives a finite-$T$ rule-of-thumb for when SPSC dominates
...
Section~\ref{sec:regret} gives the full regret analysis.
Section~\ref{sec:lower_bounds} develops the lower bounds and optimality results.
Section~\ref{sec:separations} provides mathematical separations from prior work.
Section~\ref{sec:related} reviews the literature.
...
\label{rem:eps_gap}
Theorem~\ref{thm:sqrt_ct_all} uses the information-theoretic
$\Omega(\varepsilon^2)$ per-round cost, yielding $\Omega(\sqrt{cT})$ for all policies.
Theorem~\ref{thm:probe_rate_lb} exploits the $\Omega(\varepsilon)$ per-round loss
specific to hard-projection policies, yielding $\Omega(c^{1/3}T^{2/3})$, matching
the upper bound.
Whether non-projecting algorithms can close this gap is a precise open question.
\end{remark}
...
confirming SPSC's allocation is optimal.
\end{theorem}
...
%% ============================================================
%%  8. MATHEMATICAL SEPARATIONS FROM PRIOR WORK
%% ============================================================
\section{Mathematical Separations from Prior Work}
\label{sec:separations}

We establish three formal separations showing that the identification mechanism in
this paper is not reducible to prior approaches.

\begin{proposition}[Model not reducible to task-constant models]
\label{prop:separation_qin_1}
Fix $N\ge 2$.  There exist instances of our model for which
...
With $r\ge 2$, one segment, $A_k=0$, $\Sigma_{\eta,k}=I_r$:
Proposition~\ref{prop:subspace_identification} gives
$\range(\bar{M}_k^{\mathrm{probe}})=\range(B_k^\star)$.
But with $N=1$ task, the task-covariance matrix has rank $\le 1<r$ and cannot
identify the subspace.
\end{proposition}

\begin{proof}
$\bar{S}_k^{\mathrm{probe}}=I_r$ has rank $r$, so
$\range(\bar{M}_k^{\mathrm{probe}})=\range(B_k^\star)$.
...
In the stationary setting $\theta_t=\theta$ with $\rank(\Theta)=r$,
\citet{kang2022gestt} achieve $\widetilde{O}(\sqrt{(d_1{+}d_2)MrT})$ for
bilinear bandits via an explore-then-refine approach;
\citet{pmlr-v235-jedra24a} give
$\widetilde{O}(r^{5/4}(m{+}n)^{3/4}\sqrt{T})$ via two-to-infinity recovery;
\citet{pmlr-v235-jang24e} derive arm-set-dependent regret via optimal design;
\citet{stojanovic2023spectral} provide entry-wise subspace recovery guarantees;
\citet{lu2021lowrank} achieve $\widetilde{O}(r\sqrt{T})$ via spectral methods.
All assume a \emph{fixed} $B^\star$---none handle changing representations that must
be re-identified online.
Our setting requires re-learning the subspace at each change point from scalar feedback,
...
\citet{abbasi2023dynamic} give $\widetilde{O}(\sqrt{KdT})$ for $K$-armed nonstationary
bandits;
\citet{komiyama2024adr} study ADR-bandits;
\citet{hou2024pslb} study piecewise-stationary best-arm identification.
All scale with ambient dimension~$d$, not~$r$---none exploit latent low-rank structure.
...
\citet{cai2024transfer} study minimax regret under covariate shift;
...
\begin{tabular}{lllcc}
\toprule
\textbf{Reference} & \textbf{Regret} & \textbf{Key difference} & \textbf{Prob.} & \textbf{CP?} \\
\midrule
\citet{kang2022gestt} &
$\widetilde{O}(\sqrt{(d_1{+}d_2)MrT})$ &
Stationary; no probe budget & Y & N \\
\citet{pmlr-v235-jedra24a} &
$\widetilde{O}(r^{5/4}(m{+}n)^{3/4}\sqrt{T})$ &
Stationary; no re-learning & Y & N \\
\citet{lu2021lowrank} &
$\widetilde{O}(r\sqrt{T})$ &
Stationary; learns $B$ from rewards & Y & N \\
\citet{russac2019weighted} &
$\widetilde{O}(d^{2/3}V_T^{1/3}T^{1/3})$ &
Scales with $d$; no low-rank & Y & Y \\
\citet{cheung2019learning} &
$\widetilde{O}(d^{2/3}V_T^{1/3}T^{1/3})$ &
Scales with $d$; no low-rank & Y & Y \\
\citet{abbasi2023dynamic} &
$\widetilde{O}(\sqrt{KdT})$ &
Unstructured $K$-armed; no low-rank & N & Y \\
\citet{memmel2024asid} &
Fisher-info exploration &
Control/system ID; not bandits & Y & N \\
\midrule
\textbf{SPSC (ours)} &
...
piecewise-stationary adaptation, and explicit costly probing.}
\label{tab:positioning}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Work class} & \textbf{Low-rank} & \textbf{Nonstationary} & \textbf{Costly probing} \\
\midrule
Low-rank bandits & \checkmark & \xmark & \xmark \\
Nonstationary bandits & \xmark & \checkmark & \xmark \\
Costly-information bandits & \xmark & \xmark & \checkmark \\
Nonstationary repr.\ learning & \checkmark & \checkmark & \xmark \\
...
establish that full-dimensional coverage is not an artifact of our analysis but a
fundamental information barrier.
...
The $\Omega(c^{1/3}T^{2/3})$ lower bound
(Theorem~\ref{thm:probe_rate_lb}) resolves the probe--subspace tradeoff for
projected-exploitation policies.
This opens a precise new question: can non-projecting algorithms exploit
$O(\varepsilon^2)$ per-round overhead to achieve $O(\sqrt{cT})$ total regret?
We conjecture the answer is negative under single-play scalar feedback, based on
the information geometry of the Grassmannian packing in
Theorem~\ref{thm:minimax_subspace_recovery}, but leave formal resolution as future work.
...
%% ============================================================
%%  11. SUPPORTING EXPERIMENTS
%% ============================================================
\section{Supporting Experiments}
\label{sec:experiments}

We include two experiments to validate the core theoretical predictions.
No experiment is designed to prove a theorem; each probes whether the theoretical
mechanisms operate under finite-sample conditions.
All curves report means over multiple seeds with $\pm 1$ SE bands.

\paragraph{Experiment 1: Phase-transition boundary
(Theorems~\ref{thm:closed_dynamic_regret},~\ref{thm:ambient_regret}).}
Figure~\ref{fig:phase} shows the SPSC/LinUCB regret ratio over a broad $(d,r)$ grid
($d\in\{10,\ldots,80\}$, $r\in\{1,\ldots,20\}$, $K{=}10$, $T{=}5{,}000$).
The empirical crossover boundary closely tracks the predicted
$d{-}r=T^{1/6}$ threshold from~\eqref{eq:regime_condition}: in the high-dimensional
low-rank regime ($d\ge 45$), SPSC achieves $13$--$34\%$ regret reductions, and
every $r$-curve tracks $\sqrt{r/d}$, supporting the predicted rank-dependent
improvement from $d\sqrt{T}$ to $r\sqrt{T}$.
...
\paragraph{Experiment 2: Identification condition necessity
(Theorem~\ref{thm:closed_dynamic_regret_general},
Theorem~\ref{thm:restricted_span_minimax_regret}).}
Three ablations probe the identification conditions on synthetic data
($d{=}4$, $r{=}1$, $K{=}4$, 10 seeds):
(a)~$5\times$ variance misspecification causes $<5\%$ regret change, consistent with
Proposition~\ref{prop:biased_measurement};
(b)~$\epsilon_\times{=}1$ changes regret by $<1\%$, consistent with
Corollary~\ref{cor:projector_misspec};
(c)~restricted probe coverage causes $\Omega(T)$ blow-up, confirming
Theorem~\ref{thm:restricted_span_minimax_regret}.
The first two degrade gracefully as predicted by
Theorem~\ref{thm:closed_dynamic_regret_general}; the third confirms the
