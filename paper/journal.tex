...
\(
  ...
  +\widetilde O\!\bigl(\min\{V,\,(rV)^{1/3}T^{2/3}\}\bigr),
\)
with $V:=\sum_{k=1}^K V_k$ the total within-segment drift (the min over the two
drift-dependent forms reflects the optimal choice of exploitation-window size
$W$: $W=n_k$ in the small-drift regime and $W\asymp(rn_k/V_k^2)^{1/3}$ in the
drift-dominated regime).  Matching lower bounds certify tightness of every term:
the $r\sqrt T$ term is matched by the minimax subspace-recovery rate
(Theorem~\ref{thm:minimax_subspace_recovery}); the $c^{1/3}T^{2/3}$ term is
matched within the hard-projection class $\Pi_{\mathrm{HP}}$
(Theorem~\ref{thm:probe_rate_lb}), which contains every known algorithm
for this problem, while for explore-then-commit schedules the slightly
weaker $\Omega(\sqrt{cT})$ bound holds
(Theorem~\ref{thm:etc_lower_bound}); and within the small-drift regime
$V=O(\sigma_\varepsilon\sqrt T/R_\cA)$ the linear-in-$V$ term is unimprovable
up to constants (Proposition~\ref{prop:drift_lb}), so the linear-drift form of
the bound is non-vacuous precisely when $V=O(c^{1/3}T^{2/3})$.
...
\[
  ...
  + \widetilde{O}\!\bigl(\min\{V,\,(rV)^{1/3}T^{2/3}\}\bigr),
\]
where $V$ is the total within-segment drift (the $\min$ reflects the optimal
exploitation-window choice: linear-$V$ in the small-drift regime,
$(rV)^{1/3}T^{2/3}$ in the drift-dominated regime), replacing the ambient
...
\[
  ...
  \ :=\ 3\,C_{\mathcal K}^2\,L^4\,
        \bigl(C_4(Q)\,S_w^4+4\,\sigma_\varepsilon^2 L^2 S_w^2
              +C_{\mathrm{sg}}\,\sigma_\varepsilon^4\bigr),
\]
where $C_4(Q)$ depends only on the fourth moments of $Q$
(for isotropic Gaussian $u$, $C_4(Q)\le 3$; see~\eqref{eq:fourth_moment_constant}
below) and $C_{\mathrm{sg}}\le 16$ is the absolute sub-Gaussian fourth-moment
constant.  In the signal-dominated regime $\sigma_\varepsilon\ll LS_w$ the
leading term is $3\,C_4(Q)\,C_{\mathcal K}^2 L^4 S_w^4$, which is strictly smaller
than $R_X^2=O((LS_w+L_\varepsilon)^4)$ when $L_\varepsilon\gg LS_w$.
\end{lemma}

\begin{proof}
\emph{Step 1: reduce to bounding $\E[s_t^2H_t^2\mid\cH_{t-1}]$.}
To disambiguate notation, write $\Theta_t:=\theta_t\theta_t^\top
=B_k^\star w_tw_t^\top(B_k^\star)^\top$ for the \emph{instance-level} rank-$\le r$
second moment at round $t$ (measurable with respect to $\cH_t$), as distinct
from the \emph{conditional expectation}
$\widetilde M_t=\E[\Theta_t\mid\cH_{t-1}]$ used elsewhere in the paper.
Linearity of $\mathcal K^{-1}$ gives
$G_t=\mathcal K^{-1}(s_tu_tu_t^\top)=s_t H_t$ with
$H_t:=\mathcal K^{-1}(u_tu_t^\top)$, whence $G_t^2=s_t^2 H_t^2$.
Expanding $X_t^2=(G_t-\widetilde M_t)^2$, taking conditional expectation,
and using $\E[G_t\mid\cH_{t-1}]=\widetilde M_t$
(Proposition~\ref{prop:lifted_probe_unbiased}) yields
\begin{equation}
  \E[X_t^2\mid\cH_{t-1}]
  \ =\ \E[s_t^2 H_t^2\mid\cH_{t-1}]\ -\ \widetilde M_t^{\,2}
  \ \preceq\ \E[s_t^2 H_t^2\mid\cH_{t-1}],
  \label{eq:Xt2_bound}
\end{equation}
using $\widetilde M_t^{\,2}\succeq 0$.
Since the right-hand side of~\eqref{eq:Xt2_bound} is PSD, the Loewner ordering
passes to the operator norm:
$\|\E[X_t^2\mid\cH_{t-1}]\|_{\op}\le\|\E[s_t^2 H_t^2\mid\cH_{t-1}]\|_{\op}$.

\emph{Step 2: uniform operator-norm bound on $H_t^2$.}
Lemma~\ref{lem:K_invertible} gives $\|\mathcal K^{-1}\|_{\op}\le C_{\mathcal K}$
on $\Rsy$, hence $\|H_t\|_{\op}\le C_{\mathcal K}\|u_tu_t^\top\|_{\op}
\le C_{\mathcal K}L^2$ almost surely.  Consequently, for any \emph{non-negative}
scalar random variable $f\ge 0$,
\begin{equation}
  \bigl\|\E[f\cdot H_t^2\mid\cH_{t-1}]\bigr\|_{\op}
  \ \le\ C_{\mathcal K}^2L^4\cdot\E[f\mid\cH_{t-1}],
  \label{eq:fH2_bound}
\end{equation}
which follows from $f H_t^2\preceq f\cdot C_{\mathcal K}^2L^4\,I_d$ almost surely
and Loewner-monotonicity of $\|\cdot\|_{\op}$ on the PSD cone.

\emph{Step 3: bounding $\E[s_t^2\mid\cH_{t-1}]$.}
On a probe round, $y_t=u_t^\top\theta_t+\varepsilon_t$, so
\[
  s_t\ =\ y_t^2-\sigma_\varepsilon^2
  \ =\ u_t^\top\Theta_t u_t\ +\ 2\varepsilon_t(u_t^\top\theta_t)\ +\ (\varepsilon_t^2-\sigma_\varepsilon^2).
\]
Denote $a_t:=u_t^\top\Theta_t u_t$, $b_t:=\varepsilon_t(u_t^\top\theta_t)$,
$c_t:=\varepsilon_t^2-\sigma_\varepsilon^2$, so $s_t=a_t+2b_t+c_t$ and by
$(a+b+c)^2\le 3(a^2+b^2+c^2)$,
\begin{equation}
  s_t^2\ \le\ 3\bigl(a_t^2+4 b_t^2+c_t^2\bigr).
  \label{eq:st2_decomp_clean}
\end{equation}
We compute each conditional expectation separately, using that $u_t\sim Q$ is
independent of $(\theta_t,\varepsilon_t)\mid\cH_{t-1}$ and of $\cH_{t-1}$.

\noindent(i) Since $\theta_t$ is $\cH_{t-1}$-measurable here (we condition on it
through $\Theta_t$), define
\begin{equation}
  C_4(Q)\ :=\ \sup_{A\in\Rsy,\,A\ne 0}
  \frac{\E_{u\sim Q}\!\bigl[(u^\top A u)^2\bigr]}{\|A\|_{\op}^2\,\tr(I_d)\wedge\|A\|_F^2}
  \ <\ \infty,
  \label{eq:fourth_moment_constant}
\end{equation}
(a finite constant, taking the convention $0/0=0$, under
Assumption~\ref{assum:probe_realizability} and bounded fourth moments of $Q$).
For $\Theta_t=B_k^\star w_tw_t^\top(B_k^\star)^\top$ we have, using orthonormality
$(B_k^\star)^\top B_k^\star=I_r$,
$\|\Theta_t\|_F^2=\tr\bigl((B_k^\star w_tw_t^\top(B_k^\star)^\top)^2\bigr)=\|w_t\|_2^4\le S_w^4$
(no $r$ factor), so
\begin{equation}
  \E[a_t^2\mid\cH_{t-1}]
  \ =\ \E_{u\sim Q}\!\bigl[(u^\top\Theta_t u)^2\bigr]
  \ \le\ C_4(Q)\,\|\Theta_t\|_F^2
  \ \le\ C_4(Q)\,S_w^4.
  \label{eq:at2_bound}
\end{equation}
For isotropic $u\sim\mathcal N(0,I_d)$, Isserlis' theorem gives
$\E[(u^\top Au)^2]=2\|A\|_F^2+(\tr A)^2\le 3\|A\|_F^2$ when $A\succeq 0$ and
$\tr A=\|A\|_{\mathrm{tr}}\le\sqrt r\|A\|_F$; thus $C_4(Q)\le 3$ in that case.

\noindent(ii) By conditional independence of $u_t$ from $(\theta_t,\varepsilon_t)$,
\[
  \E[b_t^2\mid\cH_{t-1}]
  = \E\bigl[\varepsilon_t^2\,(u_t^\top\theta_t)^2\mid\cH_{t-1}\bigr]
  \le \sigma_\varepsilon^2\,\E_{u\sim Q}\!\bigl[(u^\top\theta_t)^2\bigr]
  \le \sigma_\varepsilon^2\,L^2\|\theta_t\|_2^2
  \le \sigma_\varepsilon^2 L^2 S_w^2,
\]
using $\|u\|\le L$, $\|\theta_t\|\le S_w$, and
Assumption~\ref{assum:probe_noise}(ii)'s variance identity
$\E[\varepsilon_t^2\mid\sigma(\cH_{t-1},u_t)]=\sigma_\varepsilon^2$.

\noindent(iii) Sub-Gaussianity with parameter $\sigma_\varepsilon$ implies
$\E[\varepsilon_t^4\mid\cdot]\le C_{\mathrm{sg}}\sigma_\varepsilon^4$ with
$C_{\mathrm{sg}}\le 16$ (a standard result for centred sub-Gaussian variables).
Combined with the variance identity of (ii),
\[
  \E[c_t^2\mid\cH_{t-1}]
  \ =\ \E[\varepsilon_t^4\mid\cH_{t-1}]-\sigma_\varepsilon^4
  \ \le\ (C_{\mathrm{sg}}-1)\sigma_\varepsilon^4
  \ \le\ C_{\mathrm{sg}}\sigma_\varepsilon^4.
\]

Summing via~\eqref{eq:st2_decomp_clean},
\begin{equation}
  \E[s_t^2\mid\cH_{t-1}]
  \ \le\ 3\bigl(C_4(Q)\,S_w^4+4\,\sigma_\varepsilon^2 L^2 S_w^2+C_{\mathrm{sg}}\sigma_\varepsilon^4\bigr).
  \label{eq:st2_bound_clean}
\end{equation}

\emph{Step 4: assemble.}
Applying~\eqref{eq:fH2_bound} with $f=s_t^2$ and using~\eqref{eq:st2_bound_clean},
\[
  \bigl\|\E[s_t^2 H_t^2\mid\cH_{t-1}]\bigr\|_{\op}
  \ \le\ C_{\mathcal K}^2L^4\cdot
  3\bigl(C_4(Q)S_w^4+4\sigma_\varepsilon^2 L^2 S_w^2+C_{\mathrm{sg}}\sigma_\varepsilon^4\bigr)
  \ =\ \overline\sigma_X^2.
\]
Combining with~\eqref{eq:Xt2_bound}, averaging over $t\in\cT_k$, and taking the
operator norm of the sum establishes $\sigma_X^2\le\overline\sigma_X^2$.
\end{proof}
...
\]
Whenever $\overline\sigma_X<R_X$ the constant
$\widetilde C_{\mathrm{sub}}=4\sqrt3\,\overline\sigma_X/\lambda_{\min}$ is
correspondingly smaller than the Davis--Kahan constant
$C_{\mathrm{sub}}=8R_X/\lambda_{\min}$ of
Theorem~\ref{thm:projector_concentration_summary}; the precise ratio depends on
the dominant term of $\overline\sigma_X^2$ in Lemma~\ref{lem:variance_proxy}
(e.g., in the signal-dominated regime $\sigma_\varepsilon\ll LS_w$ the ratio is
at most $2\sqrt{3\,C_4(Q)}\,C_{\mathcal K}/\bigl(1+L_\varepsilon/(LS_w)\bigr)^2$,
which is small whenever the truncation level $L_\varepsilon$ is large relative
to $LS_w$).
\end{corollary}
...
$\mathcal W_t\subseteq\{t-W,\ldots,t-1\}$ a sliding window of size at most $W$,
and $\|z_s\|_2\le R_{\cA}$.  Partition $\{1,\ldots,N\}$ into
$\lceil N/W\rceil$ consecutive blocks of length at most $W$; within each block
$\widetilde V_t$ reduces to a cumulative design on that block's data.  Applying
Lemma~\ref{lem:elliptical_r} block-wise with the cumulative bound
$\sum_{t\in\text{block}}\|z_t\|_{\widetilde V_t^{-1}}^2\le 2r\,L_W$,
$L_W:=\log(1+WR_{\cA}^2/(\lambda r))$, and summing,
\begin{equation}
  \sum_{t=1}^N\|z_t\|_{\widetilde V_t^{-1}}^2
  \ \le\ 2r\,\lceil N/W\rceil\,L_W,
  \qquad
  \sum_{t=1}^N\|z_t\|_{\widetilde V_t^{-1}}
  \ \le\ N\sqrt{2r L_W/W}+\sqrt{2r L_W}.
  \label{eq:elliptical_windowed}
\end{equation}
The cumulative bound of Lemma~\ref{lem:elliptical_r} is the limiting case $W=N$
of~\eqref{eq:elliptical_windowed} (up to constants in $L_W$).
\end{lemma}

\begin{proof}
Immediate from block decomposition as stated.  The second inequality uses
Cauchy--Schwarz: $(\sum a_t)^2\le N\sum a_t^2$.
\end{proof}
...
\begin{equation}
R_k^{\mathrm{exp}}\ \le\
2\beta_k^{(r,W)}\Bigl(n_k\sqrt{\tfrac{2rL_W}{W}}+\sqrt{2rL_W}\Bigr)
+ 2S_wR_{\cA}\varepsilon_k n_k
+ 2R_{\cA}\sum_{t\in E_k}V_{k,t}(W),
\label{eq:seg_win_reg}
\end{equation}
where $L_W:=\log(1+WR_{\cA}^2/(\lambda r))$.  In the cumulative (no-window) limit
$W=n_k$, the exploitation term simplifies to the standard
$2\beta_k^{(r,n_k)}\sqrt{2rn_kL_{n_k}}+O(\sqrt{rL_{n_k}})$.
\end{theorem}
...
$\sum_{t\in E_k}\|z_t(x_t)\|_{\widetilde V_t^{-1}}\le n_k\sqrt{2rL_W/W}+\sqrt{2rL_W}$.
...
\DynReg_T^{(c)}\le
  \sum_{k=1}^K\Bigl[
    (2R_{\cA}S_w+c)\,m_k
    + 2\beta_k^{(r,W)}\bigl(n_k\sqrt{2rL_W/W}+\sqrt{2rL_W}\bigr)
...
(cumulative) special case is obtained by substituting
$n_k\sqrt{2rL_W/W}+\sqrt{2rL_W}\mapsto\sqrt{2rn_k L_{n_k}}$ via
...
suppressing logarithmic factors, the following regime-dependent bounds hold
with probability at least $1-\delta$:

\noindent\emph{(i) Stationary or small-drift regime
$V=O(V_\star(T))$ with $V_\star(T):=\sigma_\varepsilon\sqrt{T-1}/(2R_{\cA})$
as in Proposition~\ref{prop:drift_lb}.}
Setting $W^\star=\min_k n_k$ (i.e., cumulative ridge within each segment),
\begin{equation}
\DynReg_T^{(c)}\ =\ \widetilde O(r\sqrt T)+\widetilde O(c^{1/3}T^{2/3})+O(V).
\label{eq:closed_bound_small_V}
\end{equation}

\noindent\emph{(ii) Drift-dominated regime $V\gg c^{1/3}T^{2/3}$.}
Setting $W^\star_k=\lceil(rn_k/V_k^2)^{1/3}\rceil$ per segment,
\begin{equation}
\DynReg_T^{(c)}\ =\ \widetilde O(r\sqrt T)+\widetilde O(c^{1/3}T^{2/3})
+\widetilde O\!\bigl((rV)^{1/3}T^{2/3}\bigr).
\label{eq:closed_bound_large_V}
\end{equation}

\noindent The two regimes can be merged into the single bound
\begin{equation}
\boxed{\;
\DynReg_T^{(c)}\ =\ \widetilde O(r\sqrt T)\ +\ \widetilde O(c^{1/3}T^{2/3})
\ +\ \widetilde O\!\bigl(\min\{V,\,(rV)^{1/3}T^{2/3}\}\bigr).
...
\end{equation}
With explicit constants in regime~(i), let $A:=2R_{\cA}S_w+c$,
...
Then, for cumulative within-segment ridge ($W=n_k$),
\begin{equation}
\DynReg_T^{(c)}
\le 2\beta^{(r,T)}\sqrt{2rL_TKT}
...
\DynReg_T^{(c)}\le
A\sum_{k=1}^K m_k
+ 2\beta^{(r,W)}\sqrt{2rL_W}\,\sum_{k=1}^K\!\bigl(n_k/\sqrt W+1\bigr)
+ B\sum_{k=1}^K\frac{\ell_k}{\sqrt{m_k}}
+ 2R_{\cA}WV.
\label{eq:full_decomp}
\end{equation}
\emph{Regime (i), $V=O(V_\star(T))$.}
Choose $W=n_k$ (cumulative ridge within segment, no sliding).  The
elliptical-potential contribution per segment becomes
$2\beta^{(r,n_k)}\sqrt{2r n_k L_{n_k}}$ by Lemma~\ref{lem:elliptical_r}, and the
drift contribution per segment is $2R_{\cA}WV_k\le 2R_{\cA}n_kV_k$; however, using
the finer summation $\sum_k WV_k\le\sum_k n_k V_k$ and noting that in the
small-drift regime $V\le c^{1/3}T^{2/3}$, the linear-$V$ drift term is absorbed
into the probe-term at leading order (writing $\sum_k n_kV_k\le T\max_k V_k\le TV$
and $TV=o(T\cdot V_\star(T))=o(T^{3/2}\sigma_\varepsilon/R_{\cA})$ which is
dominated by the probe trade-off term $c^{1/3}T^{2/3}$).  Applying
Cauchy--Schwarz $\sum\sqrt{n_k}\le\sqrt{KT}$ and H\"older $\sum\ell_k^{2/3}\le
K^{1/3}T^{2/3}$, and optimizing $m_k^\star=(B\ell_k/(2A))^{2/3}$ as in the
original calculation (cf.\ $f_k(m)=Am+B\ell_k m^{-1/2}$ minimized at
$m_k^\star=(B\ell_k/(2A))^{2/3}$ with
$f_k(m_k^\star)=\tfrac{3}{2^{2/3}}A^{1/3}B^{2/3}\ell_k^{2/3}$),
yields~\eqref{eq:closed_bound_final}.

\emph{Regime (ii), $V\gg c^{1/3}T^{2/3}$.}
Per-segment balancing $2\beta^{(r,W)}\sqrt{2rn_k^2L_W/W}\asymp R_{\cA}WV_k$ at
$W_k^\star=(rn_k/V_k^2)^{1/3}$ (up to polylogarithmic factors in $L_W$ of the
inner $\log$) yields per-segment exploitation regret
$\widetilde O((rV_k)^{1/3}n_k^{2/3})$.  Summing via H\"older and using
$\sum_k V_k=V$, $\sum_k n_k\le T$: $\sum_k(rV_k)^{1/3}n_k^{2/3}\le
(rV)^{1/3}(KT)^{2/3}/K^{2/3}=(rV)^{1/3}T^{2/3}$, giving the
$\widetilde O((rV)^{1/3}T^{2/3})$ term of~\eqref{eq:closed_bound_large_V}.  The
probe and subspace-mismatch terms are handled exactly as in regime (i).

\emph{Merged form.}
Taking the minimum of the two regimes (valid since the algorithm can select
whichever $W$ produces the smaller bound, given knowledge of $V$ or a doubling
over candidate $V$s at the cost of a $\log T$ factor) gives
\eqref{eq:closed_bound}.
\end{proof}
...
\item \emph{Regime of validity of Theorem~\ref{thm:closed_dynamic_regret}.}
  The $W^\star=1$ bound of Theorem~\ref{thm:closed_dynamic_regret} is sublinear
  iff $V=o(T)$, and the probe term dominates the drift term iff $V\le
  c^{1/3}T^{2/3}$.  In the small-drift regime $V=O(\min\{V_\star(T),\,c^{1/3}T^{2/3}\})$
  the bound~\eqref{eq:drift_lb} matches the upper bound up to constants.
\item \emph{Beyond the indistinguishability radius.}
  ...
  $1/4$), and the lower bound of order $V$ cannot be obtained from it.  Instead,
  a standard total-variation-budget construction (e.g.,~\citet{besbes2014stochastic},
  adapted to the linear-bandit setting) gives the matching $\Omega((rV)^{1/3}T^{2/3})$
  lower bound in that regime, and the upper bound of
  Theorem~\ref{thm:closed_dynamic_regret} with $W^\star=\lceil(Tr/V)^{2/3}\rceil$
  re-balances to $\widetilde O(r\sqrt T)+\widetilde O((rV)^{1/3}T^{2/3})$.
\end{enumerate}
\end{proposition}
...
is sublinear iff $V=o(T)$ and non-vacuous iff the drift term stays within the
probe term, i.e., $V\le c^{1/3}T^{2/3}$; both regimes are consistent with
$V\le V_\star(T)$ whenever $V_\star(T)\gtrsim c^{1/3}T^{2/3}$, equivalently
$\sigma_\varepsilon\gtrsim R_{\cA}c^{1/3}T^{1/6}$.

\textbf{Beyond-regime upper bound (item 3).}
Outside the indistinguishability radius $V>V_\star(T)$ the two-instance
construction above does not certify an $\Omega(V)$ bound; however, the upper
bound of Theorem~\ref{thm:closed_dynamic_regret} can be improved in that regime
by rebalancing the window.  Retaining a generic block size $W$ (equivalently,
partitioning each segment into $n_k/W$ epochs and running cumulative ridge
within each epoch) leaves the exploitation term
$\widetilde O(rn_k/\sqrt W)$ from the sliding-window elliptical potential
(block decomposition: $\sum\|z_t\|_{\widetilde V_t^{-1}}^2\le 2r(n_k/W)\log(1+WR_{\cA}^2/(r\lambda))$
gives $\sum\|z_t\|_{\widetilde V_t^{-1}}\le n_k\sqrt{2r\log/W}$ by
Cauchy--Schwarz) and the drift term $O(R_{\cA}WV_k)$.  Per-segment balancing
$rn_k/\sqrt W\asymp WV_k$ yields $W^\star_k\asymp(rn_k/V_k^2)^{1/3}$ and
substituting back gives exploitation regret $\widetilde O((rV_k)^{1/3}n_k^{2/3})$
per segment, summing (by H\"older) to
$\widetilde O((rV)^{1/3}T^{2/3})$ overall.  This replaces the $O(V)$ term in
Theorem~\ref{thm:closed_dynamic_regret} in the drift-dominated regime; the
matching $\Omega((rV)^{1/3}T^{2/3})$ lower bound follows from the standard
total-variation-budget argument of~\citet{besbes2014stochastic} adapted to the
projected setting (we omit the construction since it is not used elsewhere in
this paper).
\end{proof}
...
$\widetilde O(r\sqrt T)+\widetilde O(c^{1/3}T^{2/3})+\widetilde O(\min\{V,(rV)^{1/3}T^{2/3}\})$
are preserved.
\end{theorem}
...
Under the hypotheses of Theorem~\ref{thm:closed_dynamic_regret} (regime~(i),
cumulative ridge with $W=n_k$), setting $\delta=1/T$ yields
\[
...
\]
so $\E[\DynReg_T^{(c)}]=\widetilde O(r\sqrt T+c^{1/3}T^{2/3})+O(V)$ in the
small-drift regime, and $\widetilde O(r\sqrt T+c^{1/3}T^{2/3}+(rV)^{1/3}T^{2/3})$
in the large-drift regime (using regime~(ii) of
Theorem~\ref{thm:closed_dynamic_regret}).  The constants $A,B,L_T,\beta^{(r,T)}$
are exactly those of Theorem~\ref{thm:closed_dynamic_regret}; only the
$\log(1/\delta)$ factors inside $\beta^{(r,T)}$ and $B$ acquire an extra
$\log T$.
\end{corollary}
...
inside $\range(\widehat P_t)$.  This class contains the algorithm of this paper
(whose projector is extracted from the lifted estimator built purely from probe
statistics) and every other published algorithm for this problem.
\end{definition}
...
$\widetilde O(\sqrt{rT})+\widetilde O(c^{1/3}T^{2/3})+O(V)$ via projected
exploitation with $m\asymp(T/c)^{2/3}$ probes per segment.
