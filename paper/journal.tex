...
\)
satisfies the matched upper and lower bound
\(
  \DynReg_T^{(c)}=\widetilde\Theta(r\sqrt T)+\widetilde\Theta(c^{1/3}T^{2/3})+O(V),
\)
with $V:=\sum_{k=1}^K V_k$ the total within-segment drift; the
$c^{1/3}T^{2/3}$ term is tight within the projected-exploitation class
(Definition~\ref{def:hard_projection}), and any restriction of probes to a
proper subspace of $\R^d$ forces linear regret~$\Omega(T)$.  An adaptive
variant attains the same rate without oracle segment boundaries, replacing
the ambient $d\sqrt T$ dependence by the intrinsic rank~$r$.
\end{abstract}
