...
\)
with $V:=\sum_{k=1}^K V_k$ the total within-segment drift.  Three separate
lower bounds certify tightness: the $r\sqrt T$ term matches the minimax
subspace-recovery rate (Theorem~\ref{thm:minimax_subspace_recovery}); the
$c^{1/3}T^{2/3}$ term is matched by a class-restricted lower bound over the
projected-exploitation class $\Pi_{\mathrm{HP}}$
(Definition~\ref{def:hard_projection},
Theorem~\ref{thm:probe_rate_lb}); and the $O(V)$ term is unimprovable
(Proposition~\ref{prop:drift_lb}), so the bound is non-vacuous precisely
in the regime $V=O(c^{1/3}T^{2/3})$.  An unconditional
$\Omega(\sqrt{cT})$ lower bound holds for every admissible policy, and any
restriction of probes to a proper subspace of $\R^d$ forces linear
regret~$\Omega(T)$.  An instance-dependent refinement replaces the
