...
(Theorem~\ref{thm:etc_lower_bound}); and the $O(V)$ term is
unimprovable (Proposition~\ref{prop:drift_lb}), so the bound is non-vacuous
precisely in the regime $V=O(c^{1/3}T^{2/3})$.
...
\[
  ...
  \ +\ S_w^4,
\]
where $C_Q$ is a constant depending only on the fourth moments of the
probe distribution $Q$ and, for isotropic Gaussian probes, scales
linearly in the ambient dimension: $C_Q\le 2C_{\mathcal{K}}^2(d+4)$.  The
$d$-dependence propagates into the constants of
Theorem~\ref{thm:variance_adaptive_concentration} and
Theorem~\ref{thm:closed_dynamic_regret} but not into the leading-rate
exponents.
\end{lemma}

\begin{proof}
Linearity of $\mathcal K^{-1}$ in its scalar-times-rank-one argument
yields
\[
  G_t\ =\ \mathcal K^{-1}(s_t u_t u_t^\top)\ =\ s_t\,H_t,
  \qquad
  H_t\ :=\ \mathcal K^{-1}(u_t u_t^\top),
\]
so $G_t^2 = s_t^2\,H_t^2$ and by unbiasedness
(Proposition~\ref{prop:lifted_probe_unbiased}),
\begin{equation}
  \E\bigl[X_t^2\bigm|\cH_{t-1}\bigr]
  \ =\ \E\bigl[G_t^2\bigm|\cH_{t-1}\bigr]\ -\ \widetilde M_t^{\,2}
  \ \preceq\ \E\bigl[s_t^2\,H_t^2\bigm|\cH_{t-1}\bigr],
  \label{eq:Xt2_bound}
\end{equation}
where we used $\widetilde M_t^{\,2}\succeq 0$ and the fact that
operator-norm variance bounds are monotone in the Loewner order.

\emph{Isotropic Gaussian case.}
For $u_t\sim\mathcal N(0,I_d)$ the identity $\mathcal K(M)=2M+\tr(M)I_d$
(from Isserlis/Stein, since
$\E[u_iu_ju_ku_\ell]=\delta_{ij}\delta_{k\ell}+\delta_{ik}\delta_{j\ell}
+\delta_{i\ell}\delta_{jk}$) inverts in closed form to
\[
  H_t\ =\ \tfrac{1}{2}\,u_tu_t^\top\ -\ \frac{\|u_t\|_2^2}{2(d+2)}\,I_d.
\]
Expanding $(H_t)^2$ gives
$H_t^2=\bigl(\tfrac14-\tfrac{1}{2(d+2)}\bigr)\|u_t\|_2^2\,u_tu_t^\top
+\tfrac{\|u_t\|_2^4}{4(d+2)^2}I_d$, and averaging over $u_t$ using the
Gaussian moments
$\E[\|u_t\|_2^2u_tu_t^\top]=(d+2)I_d$ and
$\E[\|u_t\|_2^4]=d(d+2)$ yields
$\E[H_t^2]=\tfrac{d(d+3)}{4(d+2)}\,I_d$; thus
$\|\E[H_t^2]\|_{\op}\le(d+4)/4$.

\emph{Bounding $s_t^2$.}
On probe round~$t$, $y_t=u_t^\top B_k^\star w_t+\varepsilon_t$ and
$s_t=y_t^2-\sigma_\varepsilon^2
=(u_t^\top\widetilde M_t u_t)+2(u_t^\top B_k^\star w_t)\varepsilon_t
+(\varepsilon_t^2-\sigma_\varepsilon^2)$ with $\widetilde M_t
:=B_k^\star w_tw_t^\top(B_k^\star)^\top$.  Squaring and using
$(a+b+c)^2\le 3(a^2+b^2+c^2)$,
\begin{equation}
  s_t^2\ \le\ 3\bigl[(u_t^\top\widetilde M_t u_t)^2
  +4(u_t^\top B_k^\star w_t)^2\varepsilon_t^2
  +(\varepsilon_t^2-\sigma_\varepsilon^2)^2\bigr].
  \label{eq:st2_decomp}
\end{equation}

\emph{Operator-norm bookkeeping.}
For each of the three terms in~\eqref{eq:st2_decomp}, Cauchy--Schwarz
gives
$\|\E[f(u_t,\varepsilon_t)H_t^2\mid\cH_{t-1}]\|_{\op}
\le\sqrt{\E[f^2\mid\cH_{t-1}]}\cdot\sqrt{\|\E[H_t^4]\|_{\op}}$, and for
isotropic Gaussian $u_t$, $\|\E[H_t^4]\|_{\op}=O(d^2)$ by the same
Isserlis bookkeeping (each factor of $H_t$ contributes one factor of
$\|u\|^2$ in operator norm).  Combining with
$\E[(u_t^\top\widetilde M_t u_t)^2]\le 3\|\widetilde M_t\|_F^2\le
3S_w^4 r$, $\E[(u_t^\top B_k^\star w_t)^2]
\le L^2\lambda_{\max}(B_k^\star\bar S_k^{\mathrm{probe}}(B_k^\star)^\top)$
up to $Q$-dependent absolute constants, and $\E[(\varepsilon_t^2-
\sigma_\varepsilon^2)^2]\le c_\varepsilon\sigma_\varepsilon^4$
(sub-Gaussian $\varepsilon_t$), one obtains
\[
  \bigl\|\E[s_t^2 H_t^2\mid\cH_{t-1}]\bigr\|_{\op}
  \ \le\ C_Q L^2\bigl(\sigma_\varepsilon^2+S_w^2
  \lambda_{\max}(B_k^\star\bar S_k^{\mathrm{probe}}(B_k^\star)^\top)\bigr)
  \ +\ S_w^4,
\]
with $C_Q\le 2C_{\mathcal K}^2(d+4)$ absorbing all absolute constants and
the $d+4$ factor from $\|\E[H_t^2]\|_{\op}$.  Averaging
over $t\in\cT_k$ and taking the operator norm of the sum establishes
$\sigma_X^2\le\overline\sigma_X^2$ as claimed.

\emph{General $Q$.}
For a general probe distribution $Q$ satisfying
Assumption~\ref{assum:probe_realizability} and bounded fourth moments,
$\mathcal K$ remains invertible on the symmetric matrices and the same
bookkeeping applies with $\|\E[H_t^2]\|_{\op}\le C_{\mathcal K}^2\|\E_Q
[(u^\top M u)uu^\top]\|_{\op}/\|M\|_{\op}$ controlled by the fourth
moments of $Q$; the constant $C_Q$ then depends on those moments (but
not on the LDS parameters) and inherits an $O(d)$ ambient-dimension
factor from the trace-correction structure of $\mathcal K^{-1}$.
\end{proof}
...
\]
In the regime $\sigma_\varepsilon\ll LS_w\sqrt{\lambda_{\min}}$, the
constant $\widetilde C_{\mathrm{sub}}$ is smaller than the Davis--Kahan
constant $C_{\mathrm{sub}}=8R_X/\lambda_{\min}$ of
...
\label{eq:drift_lb}
\end{equation}
Since $\widetilde{\mathcal M}_V$ strictly contains the restriction of our
stochastic-LDS model to the drift-$V$ sub-family (the latter is recovered
as $\rho(A_k)\to 1^-$ by continuity of the regret functional in
$\rho$), the same bound holds as an \emph{a~fortiori} lower bound
whenever the upper bound is claimed over stochastic LDS with
$\rho(A_k)\le 1-\alpha$ and bounded total-variation budget $V$; the
constant $\tfrac{1}{8}$ is preserved up to an $\alpha$-dependent factor
that tends to $1$ as $\alpha\downarrow 0$.
Consequently:
...
This realisation lies in the enlarged class $\widetilde{\mathcal M}_V$
(marginally stable: one may write $w_{t+1}=-w_t$ with $A_k=-1$ and
$\eta_t\equiv 0$, so $\rho(A_k)=1$).  The rescaling $\Delta/2\le S_w$
ensures $|w_t^j|\le S_w$.  Assumption~\ref{assum:probe_noise} and the
observation model are otherwise unchanged, so $\nu^{\pm}$ are valid
instances of $\widetilde{\mathcal M}_V$.
